...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special summon 1 machine monster from hand or RP
	...
	c:RegisterEffect(e2)
	--If correctly calls opponent's drawn card, send this card to RP to draw 1
...
function s.cfilter(c,tp)
	return c:IsMonster() and c:GetControler()~=c:GetOwner() and c:IsAbleToRestAsCost() and Duel.GetMZoneCount(tp,c)>0
end
	--Send 1 monster you control, owned by your opponent, to RP
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST|LOCATION_HAND,0,1,nil,e,tp) end
