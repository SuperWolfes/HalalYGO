...
function s.initial_effect(c)
	-- Send "Jack's Knight", "Queen's Knight", and "King's Knight" to the RP
...
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local summg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_EXTRA|LOCATION_REST,0,nil,e,tp)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOREST,nil,3,tp,LOCATION_MZONE|LOCATION_HAND)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_EXTRA|LOCATION_REST)
end
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	local summg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_EXTRA|LOCATION_REST,0,nil,e,tp)
