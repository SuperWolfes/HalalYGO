...
function s.thfilter(c)
	return (c:IsCode(CARD_DESTRUDIC_BOARD) or c:IsRace(RACE_TAINTED) and c:GetLevel()==8) and c:IsAbleToHand()
end
...
function s.tdfilter(c)
	return (c:IsCode(CARD_DESTRUDIC_BOARD) or c:IsSetCard(0x1c)) and c:IsAbleToDeck()
end
function s.tdrtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(s.tdfilter,tp,LOCATION_HAND+LOCATION_REST,0,nil)
...
function s.tdrop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.tdfilter),tp,LOCATION_HAND+LOCATION_REST,0,nil)
