...
function s.filter1(c,tp)
	local lv=c:GetLevel()
	return lv>0 and c:IsFaceup() and Duel.IsExistingTarget(s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,nil,lv)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g2=Duel.SelectTarget(tp,s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,nil,g1:GetFirst():GetLevel())
