...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Set 1 "Sinful Spoils" Actional/Trap from the RP
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local ct=Duel.GetMatchingGroupCount(s.sinfilter,tp,LOCATION_REST|LOCATION_REMOVED,0,nil)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local sg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.sinfilter),tp,LOCATION_REST|LOCATION_REMOVED,0,nil)
