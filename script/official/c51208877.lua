...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- Add 1 Level 4 EARTH Wanderer monster to the hand
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #tg>0 and Duel.Destroy(tg,REASON_EFFECT)>0 then
		...
		if dam>0 and not Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REST,0,1,nil,CARD_EXCHANGE_GUARDIAN) then
			Duel.BreakEffect()
...
function s.thfilter(c)
	return c:IsLevel(4) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsRace(RACE_WANDERER) and c:IsAbleToHand()
end
