...
function s.costfilter(c,tp)
	return c:IsSetCard(0x47) and c:IsMonster() and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
		and Duel.IsExistingTarget(s.tgfilter,tp,LOCATION_REST,0,1,c)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local cg=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_MZONE|LOCATION_REST,0,1,1,nil,tp)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,s.tgfilter,tp,LOCATION_REST,0,1,1,nil)
