...
function s.rescon(sg,e,tp,mg)
	return sg:GetClassCount(Card.GetLocation)==2 and sg:IsExists(Card.IsSetCard,1,nil,0x1f) and sg:IsExists(Card.IsSetCard,1,nil,0x3008)
		...
		and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK+LOCATION_REST,0,1,sg,e,tp)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if sc and Duel.SpecialSummon(sc,0,tp,tp,false,false,POS_FACEUP)>0 then
		...
		local tg=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.thfilter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil)
		if #tg>0 then
