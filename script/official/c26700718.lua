...
function s.initial_effect(c)
	-- Place 1 "Diabellestar" monster in its owner's Actional/Trap Zone
...
function s.plfilter(c,tp,hand_chk)
	if not (c:IsMonster() and c:IsSetCard(SET_DIABELLESTARR) and not c:IsUnliked()) then return false end
...
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,nil,tp,hand_chk) end
end
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.plfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,1,nil):GetFirst()
