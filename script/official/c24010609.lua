...
function s.actop(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_ACTIONAL) then
...
function s.regop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if re:GetHandler():IsSetCard(0x115) and re:IsActiveType(TYPE_ACTIONAL) and rp==tp and re:IsHasType(EFFECT_TYPE_ACTIVATE) then
...
function s.setfilter(c)
	return c:IsSetCard(0x115) and c:IsActional() and c:IsSSetable()
end
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=e:GetHandler():GetFlagEffectLabel(id)
	if chk==0 then return ct and ct>0 and Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_REST,0,1,nil) end
...
function s.setop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.setfilter),tp,LOCATION_REST,0,nil)
