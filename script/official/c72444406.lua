...
function s.initial_effect(c)
	...
	--Place 1 monster from your RP or your opponent's field in the Actional & Trap Zone as a Continuous Trap
	...
	c:RegisterEffect(e2)
	--Special Summon 1 Monster Card from your Actional & Trap Zone
...
function s.plfilter(c)
	...
	return c:IsMonster() and c:IsFaceup() and Duel.GetLocationCount(owner_p,LOCATION_SZONE)>0 and not c:IsUnliked()
		and c:CheckUniqueOnField(owner_p)
end
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return s.plfilter(chkc) and ((chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST))
	...
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil,REASON_EFFECT)
		and Duel.IsExistingTarget(s.plfilter,tp,LOCATION_REST,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectTarget(tp,s.plfilter,tp,LOCATION_REST,LOCATION_MZONE,1,1,nil):GetFirst()
