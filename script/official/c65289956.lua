...
function s.initial_effect(c)
	--Set 1 Actional/Trap from your hand or RP
...
function s.setconfilter(c)
	return c:IsActionalTrap() and c:IsReason(REASON_EFFECT)
end
...
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_HAND|LOCATION_REST,0,1,nil) end
end
function s.vetop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
	local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.setfilter),tp,LOCATION_HAND|LOCATION_REST,0,1,1,nil):GetFirst()
