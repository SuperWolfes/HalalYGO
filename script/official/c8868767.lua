...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SUFFICE+CATEGORY_REMOVE+CATEGORY_DECKDES)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		local res=false
		if turn_p==tp then
			res=Duel.IsExistingMatchingCard(s.rmfilter,tp,LOCATION_REST+LOCATION_MZONE,0,1,nil)
		else
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetLabel()==tp then
		...
		local rg=Duel.SelectMatchingCard(tp,s.rmfilter,tp,LOCATION_REST+LOCATION_MZONE,0,res,res,nil)
		...
		if Duel.DiscardDeck(tp,res,REASON_EFFECT)==res and res==6 then
			...
			if og:FilterCount(Card.IsLocation,nil,LOCATION_REST)~=res then return end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
			local rg=Duel.SelectMatchingCard(tp,s.rmfilter,tp,LOCATION_REST+LOCATION_MZONE,0,1,1,nil)
			if #rg==0 then return end
			Duel.BreakEffect()
