...
function s.filter(c,e,tp)
	local rg=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_MZONE+LOCATION_REST,0,c)
	local lv=c:GetLevel()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.GetMatchingGroup(s.filter,tp,LOCATION_HAND+LOCATION_REST,0,nil,e,tp)
	local lvt={}
	local pc=1
	for i=2,12 do
	...
	local rg=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_MZONE+LOCATION_REST,0,nil)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_REST+LOCATION_HAND,0,1,1,nil,e,tp,e:GetLabel())
	local tc=g:GetFirst()
	if tc then
