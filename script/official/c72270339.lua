--黒魔女ディアベルスター
--Diabellstar the Black Mint
--Scripted by Satellaa
local s,id=GetID()
function s.initial_effect(c)
	--Special Summon this card (from your hand) by sending 1 card from your hand or field to the RP
	...
	c:RegisterEffect(e1)
	--Set 1 "Sinful Spoils" Actional/Trap directly from your Deck
	...
	c:RegisterEffect(e3)
	--Send 1 card from your hand or field to the RP, and if you do, Special Summon this card
...
function s.tgfilter(c,tp,bool)
	local tg_check=nil
	if bool then
		tg_check=c:IsAbleToRest()
	else
		tg_check=c:IsAbleToRestAsCost()
	end
...
function s.setfilter(c)
	return c:IsSetCard(SET_SINFUL_SPOILS) and c:IsActionalTrap() and c:IsSSetable()
end
