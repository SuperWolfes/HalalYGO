...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetMatchingGroupCount(Card.IsMonster,tp,LOCATION_REST,0,nil)>Duel.GetMatchingGroupCount(Card.IsMonster,1-tp,LOCATION_REST,0,nil)
end
...
function s.atkfilter(c)
	return c:IsSetCard(0xa4) and c:IsMonster() and (c:IsLocation(LOCATION_REST) or c:IsFaceup())
end
function s.val(e,c)
	return Duel.GetMatchingGroupCount(s.atkfilter,c:GetControler(),LOCATION_MZONE+LOCATION_REST,0,nil)*300
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local locs=LOCATION_REST+LOCATION_HAND
	if chk==0 then
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.SendtoHand(c,nil,REASON_EFFECT)>0 then
		...
		local locs=LOCATION_REST+LOCATION_HAND
		local g1=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.spfilter),tp,locs,0,nil,e,tp,44632120)
		local g2=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.spfilter),tp,locs,0,nil,e,tp,71036835)
		local g3=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.spfilter),tp,locs,0,nil,e,tp,7021574)
		local g4=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.spfilter),tp,locs,0,nil,e,tp,34419588)
		local g5=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.spfilter),tp,locs,0,nil,e,tp,CARD_KURIBOH)
