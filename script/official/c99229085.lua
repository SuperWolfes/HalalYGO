...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Grant a "Special Summon 1 monster from your opponent's RP to their field" effect to a "Gimmick Puppet" Xyz Monster that has this card as material
...
function s.oppsptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(1-tp) and s.oppspfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE,tp)>0
		and Duel.IsExistingTarget(s.oppspfilter,tp,0,LOCATION_REST,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.oppspfilter,tp,0,LOCATION_REST,1,1,nil,e,tp)
