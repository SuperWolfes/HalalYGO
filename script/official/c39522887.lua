...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Apply effect to a face-up card
...
function s.spfilter(c)
	return c:IsMonster() and c:IsRace(RACE_FISH) and c:IsAbleToRemoveAsCost()
		and (c:IsLocation(LOCATION_HAND) or aux.SpElimFilter(c,true))
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local rg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND|LOCATION_REST|LOCATION_MZONE,0,e:GetHandler())
