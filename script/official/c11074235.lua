...
function s.setfilter(c)
	return c:IsSpellTrap() and not c:IsType(TYPE_FIELD) and (c:IsLocation(LOCATION_REST) or c:IsFaceup()) and c:IsSSetable()
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.setfilter),tp,LOCATION_REST+LOCATION_REMOVED,0,nil)
