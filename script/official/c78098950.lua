--刀皇－都牟羽沓薙
--Tsumuha-Kutsunagi the Watcher of Swords
...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Opponent can send cards to the RP
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(id,1))
	e2:SetCategory(CATEGORY_TOREST+CATEGORY_DRAW+CATEGORY_TODECK)
...
function s.gytg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TODECK,nil,1,PLAYER_ALL,LOCATION_REMOVED|LOCATION_ONFIELD|LOCATION_REST)
end
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsAbleToRest,tp,0,LOCATION_ONFIELD,nil)
	if #g>0 and Duel.IsPlayerCanDraw(tp,1) and Duel.IsPlayerCanDraw(1-tp,1)
		...
		local oc=Duel.SendtoRest(sg,REASON_EFFECT,PLAYER_NONE,1-tp)
		if oc>0 then
	...
	--Shuffle all cards that are banished, on the field, and in the RPs into the Deck
...
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	local loc=LOCATION_ONFIELD|LOCATION_REMOVED|LOCATION_REST
	local g=Duel.GetMatchingGroup(aux.RestValleyFilter(Card.IsAbleToDeck),tp,loc,loc,nil)
