...
function s.initial_effect(c)
	--Special Summon 1 "Atlantean" or "Mermail" monster from your hand or RP, and if you do, equip it with this card
	...
	c:RegisterEffect(e2)
	--Shuffle up to 3 of your 3 Fish, Sea Serpent, and/or Aqua monsters from your RP or banishment into your Deck
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST|LOCATION_REMOVED) and s.tdfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_REST|LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,s.tdfilter,tp,LOCATION_REST|LOCATION_REMOVED,0,1,3,nil)
