--海皇龍神 ポセイドラ・アビス
--Poseidra Abyss, the Atlantean Dragon Being
--scripted by Naim
local s,id=GetID()
function s.initial_effect(c)
	...
	--Special Summon 3 Level 3 or lower Fish, Sea Serpent, and/or Aqua monsters from your hand and/or RP.
...
function s.thcostfilter(c)
	return c:IsAttribute(ATTRIBUTE_WATER) and c:IsAbleToRestAsCost()
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN)
		...
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND|LOCATION_REST,0,3,nil,e,tp) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_REST,0,3,3,nil,e,tp)
	if #g==3 then
