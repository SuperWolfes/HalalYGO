...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ft>0 then
		...
		local g1=Duel.SelectMatchingCard(1-tp,aux.NecroValleyFilter(s.spfilter1),1-tp,LOCATION_REST,0,1,ft,nil,e,1-tp)
		if #g1>0 then
			...
			if zone~=0 and ct>0 and c:IsRelateToEffect(e) then
				...
				local g2=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter2),tp,LOCATION_REST,0,1,ct,nil,e,tp,zone)
				if #g2>0 then
					Duel.BreakEffect()
