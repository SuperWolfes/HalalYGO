...
function s.initial_effect(c)
	--Negate the effects of cards your opponent controls, up to the number of "Dark Fusion" and Actionals that mention it in your RP
	...
	c:RegisterEffect(e1)
	--Shuffle up to 5 "HERO" monsters from your RP and/or banishment into the Deck
...
function s.disctfilter(c)
	return c:IsCode(CARD_DARK_FUSION) or (c:IsActional() and c:ListsCode(CARD_DARK_FUSION))
end
function s.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and chkc:IsNegatable() end
	local ct=Duel.GetMatchingGroupCount(s.disctfilter,tp,LOCATION_REST,0,nil)
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,s.tdfilter,tp,LOCATION_REST|LOCATION_REMOVED,0,1,5,nil)
