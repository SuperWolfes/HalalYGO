...
function s.tofieldop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and aux.RemoveUntil(c,nil,REASON_EFFECT,PHASE_STANDBY,id,e,tp,s.returnop) and c:IsLocation(LOCATION_REMOVED) then
		...
		local tc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.tofieldfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,1,nil):GetFirst()
		...
		if Duel.IsExistingMatchingCard(aux.RestValleyFilter(s.cfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,nil,e,tp,ft) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(id,2))
			local sc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.cfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,1,nil,e,tp,ft):GetFirst()
