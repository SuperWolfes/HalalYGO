...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--"Ancient Gear Gopal" and monsters that mention it are unaffected by your opponent's activated monster effects
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetCode(EFFECT_IMMUNE_EFFECT)
	e2:SetTargetRange(LOCATION_MZONE,0)
	e2:SetRange(LOCATION_SZONE)
	e2:SetTarget(function(e,c) return c:IsCode(CARD_ANCIENT_GEAR_GOPAL) or c:ListsCode(CARD_ANCIENT_GEAR_GOPAL) end)
	e2:SetValue(s.immval)
	c:RegisterEffect(e2)
	--Fusion Summon 1 Fusion Monster that mentions "Ancient Gear Gopal"
	local params={fusfilter=function(c) return c:ListsCode(CARD_ANCIENT_GEAR_GOPAL) end,
...
function s.aggfilter(c,tp)
	return c:IsCode(CARD_ANCIENT_GEAR_GOPAL) and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp)
end
