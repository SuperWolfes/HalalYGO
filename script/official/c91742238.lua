--リターン・オブ・アンデット
--Return of the Contaminateds
--Scripted by Eerie Code

local s,id=GetID()
function s.initial_effect(c)
	--Banish 1 contaminated monster from field, then controller of it special summons 1 contaminated monster from their GY
...
function s.rmfilter(c,e,tp)
	...
	return c:IsFaceup() and c:IsRace(RACE_CONTAMINED) and c:IsAbleToRemove()
		...
		and Duel.IsExistingMatchingCard(s.spfilter,p,LOCATION_REST,0,1,nil,e,tp,p)
end
function s.spfilter(c,e,tp,p)
	return c:IsRace(RACE_CONTAMINED) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE,p)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if rc and Duel.Remove(rc,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),p,LOCATION_REST,0,1,1,nil,e,tp,p)
