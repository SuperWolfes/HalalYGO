...
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and Duel.MoveToField(c,tp,tp,LOCATION_PZONE,POS_FACEUP,true)
		and Duel.IsExistingMatchingCard(aux.RestValleyFilter(s.tefilter),tp,LOCATION_REST,0,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(id,3)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SELECT)
		local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.tefilter),tp,LOCATION_REST,0,1,1,nil)
		if #g>0 then
			Duel.BreakEffect()
