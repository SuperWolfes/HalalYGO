...
function s.initial_effect(c)
	...
	e1:SetCondition(function(e,tp) return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsRace,RACE_TAINTED|RACE_ILLUSION|RACE_MENTOR),tp,LOCATION_MZONE,0,1,nil) end)
...
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_TAINTED|RACE_ILLUSION|RACE_MENTOR) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.effop(e,tp,eg,ep,ev,re,r,rp,chk)
	local op=e:GetLabel()
	if op==1 then
		--Special Summon 1 Tainted, Illusion, or Mentor monster from your hand
