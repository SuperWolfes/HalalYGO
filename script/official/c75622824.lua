...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		e:SetLabel(0)
		return ct>0 and (ct==1 or not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN)) 
			and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,2000,1200,5,RACE_MENTOR,ATTRIBUTE_WIND)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if not Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,2000,1200,5,RACE_MENTOR,ATTRIBUTE_WIND) then return end
	for i=1,ct do
