...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE|LOCATION_REST) and s.texfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.texfilter,tp,LOCATION_MZONE|LOCATION_REST,LOCATION_MZONE|LOCATION_REST,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,s.texfilter,tp,LOCATION_MZONE|LOCATION_REST,LOCATION_MZONE|LOCATION_REST,1,1,nil)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_EXTRA) then
		...
		local g1=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.albazfilter),tp,LOCATION_REST,0,nil,e,tp)
		local g2=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.spfilter),tp,0,LOCATION_REST,nil,e,tp)
