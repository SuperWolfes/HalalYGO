...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoDeck(tc,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_EXTRA) then
		...
		local g1=Duel.GetMatchingGroup(aux.RestValleyFilter(s.albazfilter),tp,LOCATION_REST,0,nil,e,tp)
		local g2=Duel.GetMatchingGroup(aux.RestValleyFilter(s.spfilter),tp,0,LOCATION_REST,nil,e,tp)
