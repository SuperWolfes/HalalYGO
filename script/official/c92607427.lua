...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add 1 Level 4 or lower DARK monster rom your RP to your hand
	...
	e2:SetCondition(function(e,tp) return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsRace,RACE_WANDERER),tp,LOCATION_MZONE,0,1,nil) end)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		--It can attack again in a row each time it destroys a monster by battle and sends it to the RP
		local e1=Effect.CreateEffect(c)
...
function s.chainatkcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsRelateToBattle() and c:CanChainAttack(0) and bc:IsLocation(LOCATION_REST) and bc:IsMonster()
end
