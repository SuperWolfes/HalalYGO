...
function s.tgfilter(c)
	return c:IsRace(RACE_TAINTED) and c:IsType(TYPE_TUNER) and c:IsAbleToGrave()
end
...
function s.lvop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (sc and Duel.SendtoGrave(sc,REASON_EFFECT)>0 and sc:IsLocation(LOCATION_REST)) then return end
