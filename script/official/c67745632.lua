...
function s.initial_effect(c)
	...
	--Add 1 Dinosaur monster from the RP to the hand
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if e:GetHandler():IsSummonLocation(LOCATION_REST) then
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoHand(tc,nil,REASON_EFFECT)>0
		and tc:IsLocation(LOCATION_HAND) and e:GetHandler():IsSummonLocation(LOCATION_REST) then
		Duel.ShuffleHand(tp)
