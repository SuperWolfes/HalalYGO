...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Place "Mileon" monsters that are destroyed in the S/T zone instead of sending them to the RP
...
function s.dplfilter(c,tp)
	return c:IsSetCard(SET_MILEON) and s.hplfilter(c,tp)
end
...
function s.repfilter(c,tp)
	return c:IsSetCard(SET_MILEON) and c:IsFaceup() and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) 
