...
function s.disrmcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return race&RACE_CONTAMINED>0 and code1~=id and code2~=id
end
function s.filter(c)
...
function s.disrmtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b2=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.filter),tp,LOCATION_MZONE+LOCATION_REST,LOCATION_MZONE+LOCATION_REST,1,nil)
		and Duel.GetFlagEffect(tp,id+1)==0
	if chk==0 then return b1 or b2 end
	Duel.SetPossibleOperationInfo(0,CATEGORY_DISABLE,eg,1,0,0)
	Duel.SetPossibleOperationInfo(0,CATEGORY_REMOVE,nil,1,PLAYER_EITHER,LOCATION_MZONE+LOCATION_REST)
end
function s.disrmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==0 then
		...
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.filter),tp,LOCATION_MZONE+LOCATION_REST,LOCATION_MZONE+LOCATION_REST,1,1,nil)
