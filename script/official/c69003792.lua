...
function s.initial_effect(c)
	...
	e1:SetTarget(s.target(Locked.Target(ritparams),Locked.Operation(ritparams)))
	c:RegisterEffect(e1)
end
...
function s.target(rtg,rtop)
	return function(e,tp,eg,ep,ev,re,r,rp,chk)
		local b1=Duel.GetMatchingGroupCount(Card.IsFaceup,tp,0,LOCATION_MZONE,nil)>0
			and Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsType,TYPE_LOCKED),tp,LOCATION_MZONE,LOCATION_MZONE,nil)>0
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local atk=Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsType,TYPE_LOCKED),tp,LOCATION_MZONE,LOCATION_MZONE,nil)*(-500)
