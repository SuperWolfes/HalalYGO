...
function s.thspfilter(c,e,tp,ft)
	return c:IsLevel(1) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsFaceup()
...
function s.cfilter(c,tp,fcoreed)
	return c:IsFaceup() and c:IsAbleToGraveAsCost() and Duel.GetMZoneCount(tp,Group.FromCards(c,fcoreed))>0
end
