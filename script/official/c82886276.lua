...
function s.initial_effect(c)
	--Special Summon monsters from the RP
...
function s.tgfilter(c,e,tp)
	return c:IsSetCard(SET_FIREWALL) and c:IsLinkMonster()
		and (Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST,0,1,nil,e,tp,c,c:GetLinkedZone(tp),tp)
		or Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST,0,1,nil,e,tp,c,c:GetLinkedZone(1-tp),1-tp))
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_REST,0,nil,e,tp,tc,tc:GetLinkedZone(tp),tp)
	local g2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_REST,0,nil,e,tp,tc,tc:GetLinkedZone(1-tp),1-tp)
	while #(g1+g2)>0 do
		...
		g1=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_REST,0,nil,e,tp,tc,tc:GetLinkedZone(tp),tp)
		g2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_REST,0,nil,e,tp,tc,tc:GetLinkedZone(1-tp),1-tp)
