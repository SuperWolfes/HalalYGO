...
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Make the opponent send a monster to the RP
	...
	c:RegisterEffect(e1)
	--Locked monsters you control inflict piercing damage
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_FIELD)
	e2:SetCode(EFFECT_PIERCE)
	e2:SetRange(LOCATION_MZONE)
	e2:SetTargetRange(LOCATION_MZONE,0)
	e2:SetTarget(aux.TargetBoolFunction(Card.IsType,TYPE_LOCKED))
	c:RegisterEffect(e2)
	--Add 1 Locked Monster or "Machine Angel Locked" to the hand
...
function s.thfilter(c)
	return (c:IsCode(39996157) or c:IsLockedMonster()) and c:IsAbleToHand()
end
