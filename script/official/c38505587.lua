...
function s.initial_effect(c)
	...
	c:RegisterEffect(e3)
	--Special Summon 1 other "Morequesthmallon" and inflict 1000 damage
	...
	e4:SetTarget(s.questhsptg)
	e4:SetOperation(s.questhspop)
	c:RegisterEffect(e4)
end
s.listed_names={CARD_SHINING_SARCOPHAGUS,id}
local LOCATIONS_REMOVED_HAND_DECK_REST=LOCATION_REMOVED|LOCATION_HAND|LOCATION_DECK|LOCATION_REST
...
function s.questhspfilter(c,e,tp)
	return c:IsCode(id) and (c:IsFaceup() or not c:IsLocation(LOCATION_REMOVED)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.questhsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.questhspfilter,tp,LOCATIONS_REMOVED_HAND_DECK_REST,0,1,e:GetHandler(),e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATIONS_REMOVED_HAND_DECK_REST)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1000)
end
function s.questhspop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	local c=e:GetHandler()
	local exc=c:IsRelateToEffect(e) and c or nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.questhspfilter),tp,LOCATIONS_REMOVED_HAND_DECK_REST,0,1,1,exc,e,tp)
