...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	-- Send 1 Beast, Beast-Warrior, or Winged Beast monster to the RP
...
function s.spcostfilter(c)
	return c:IsSetCard(SET_TRI_BRIGADE) and c:IsActionalTrap()
end
function s.spcost(e,c,sp,st)
	return not c:IsLocation(LOCATION_EXTRA) or Duel.GetMatchingGroupCount(s.spcostfilter,sp,LOCATION_REST,0,nil)>2
end
...
function s.tgfilter(c)
	return c:IsRace(RACES_BEAST_BWARRIOR_WINGB) and c:IsAbleToRest()
end
