...
function s.filter(c)
	return c:IsRace(RACE_MENTAL) and c:IsLevel(3) and (c:IsFaceup() or not c:IsOnField())
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and chkc:IsAbleToHand() end
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_REST+LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingTarget(Card.IsAbleToHand,tp,0,LOCATION_ONFIELD,1,nil) end
	local ct=Duel.GetMatchingGroup(s.filter,tp,LOCATION_REST+LOCATION_MZONE,0,nil):GetClassCount(Card.GetCode)
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0x173) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
