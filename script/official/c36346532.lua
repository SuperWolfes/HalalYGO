...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Send 1 Set card on the field to the RP and Set 1 "Paleozoic" Trap card directly from the Deck
	...
	c:RegisterEffect(e2)
	--Mismatching replacement for Set card(s)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
	e3:SetCode(EFFECT_DESTROY_REPLACE)
	e3:SetRange(LOCATION_REST|LOCATION_MZONE)
...
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and Duel.GetOperatedGroup():GetFirst():IsLocation(LOCATION_REST)
