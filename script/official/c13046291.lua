...
function s.atchcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsAbleToRemoveAsCost()
		...
		and Duel.IsExistingMatchingCard(Card.IsRace,tp,LOCATION_HAND+LOCATION_REST,0,1,nil,RACE_REPTILE+RACE_DINOSAUR) end
...
function s.atchop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and not tc:IsImmuneToEffect(e)
		and Duel.IsExistingMatchingCard(Card.IsRace,tp,LOCATION_HAND+LOCATION_REST,0,1,nil,RACE_REPTILE+RACE_DINOSAUR) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
		local g=Duel.GetMatchingGroup(Card.IsRace,tp,LOCATION_HAND+LOCATION_REST,0,nil,RACE_REPTILE+RACE_DINOSAUR)
