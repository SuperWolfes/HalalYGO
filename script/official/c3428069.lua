--破壊剣の使い手－バスター・ブレイダー
--Buster Blader, the Mismatching Swordmaster
local s,id=GetID()
...
function s.filter(c,e,tp)
	return c:IsMonster() and c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(1-tp)
		and c:IsLocation(LOCATION_REST) and c:IsReason(REASON_EFFECT+REASON_BATTLE) and c:IsCanBeEffectTarget(e) and not c:IsForbidden()
end
