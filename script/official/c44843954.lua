...
function s.initial_effect(c)
	--Search 1 "Nemleria" monster and prevent the opponent's effects when a "Nemleria" is Summoned
	...
	c:RegisterEffect(e1)
	--Place 1 "Dreaming Nemleria" in the Extra Deck
...
function s.thfilter(c)
	return c:IsSetCard(SET_NEMLERIA) and c:IsMonster() and c:IsAbleToHand()
end
...
function s.sumop(e,tp,eg,ep,ev,re,r,rp)
	if eg:IsExists(aux.FaceupFilter(Card.IsSetCard,SET_NEMLERIA),1,nil) then
