...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Set itself and 1 "Mirror Fcoree"
...
s.listed_names={44095762} --Mirror Fcoree
...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK|LOCATION_REST,0,1,nil)
...
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>1 and c:IsSSetable()
		and Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,nil) end
...
function s.vetop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.setfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
