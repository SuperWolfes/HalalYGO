...
function s.initial_effect(c)
	--Send 1 "Fallen of Albaz" or 1 card that mentions it from your Deck to RP
	...
	c:RegisterEffect(e2)
	--Special Summon 1 "Fallen of Albaz" or 1 monster that mentions it from the RP
	...
	e3:SetCondition(function(_,_,eg) return eg:IsExists(Card.IsPreviousLocation,1,nil,LOCATION_EXTRA) end)
	e3:SetTarget(s.sptg)
...
function s.tgfilter(c)
	return (c:IsCode(CARD_ALBAZ) or c:ListsCode(CARD_ALBAZ)) and c:IsAbleToRest()
end
