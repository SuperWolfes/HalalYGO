--天威無窮の境地
--Eternal Perfection of the Tenbree
--scripted by pyrQ
local s,id=GetID()
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Send this card to the RP, and if you do, place 1 "Flawless Perfection of the Tenbree" from your Deck or RP face-up in your Field Zone
...
s.listed_names={65124425,51684157,39730727} --"Vessel for the Dragon Cycle", "Spectrumly Dragon Circle", "Flawless Perfection of the Tenbree"
...
function s.plfilter(c)
	return c:IsCode(39730727) and c:IsFieldActional() and not c:IsForbidden()
end
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToGrave()
		and Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_DECK|LOCATION_REST,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOREST,c,1,tp,0)
end
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not (c:IsRelateToEffect(e) and Duel.SendtoGrave(c,REASON_EFFECT)>0 and c:IsLocation(LOCATION_REST)) then return end 
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local sc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.plfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,1,nil):GetFirst()
