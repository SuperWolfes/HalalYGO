...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local hyddennt_chk=Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_R_ACE_HYDDENNT),tp,LOCATION_ONFIELD,0,1,nil)
	if chkc then return chkc:IsLocation(LOCATION_REST) and (chkc:IsControler(tp) or hyddennt_chk) and s.spfilter(chkc,e,tp) end
	local loc=hyddennt_chk and LOCATION_REST or 0
