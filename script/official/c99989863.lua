...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Send 1 "Taintedsmith" monster from your Deck or Extra Deck to the RP
...
function s.trgtfilter(c,rc)
	return c:IsLevelAbove(7) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_TAINTED) and c:IsFaceup()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOREST,tgg,#tgg,tp,0)
end
...
function s.tgfilter(c)
	return c:IsSetCard(SET_TAINTEDSMITH) and c:IsMonster() and c:IsAbleToGrave()
end
