...
function s.dtop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.dtfilter),tp,LOCATION_HAND+LOCATION_REST,0,1,1,nil):GetFirst()
	if tc then
...
function s.tdfilter(c)
	return (c:IsFaceup() or c:IsLocation(LOCATION_REST)) and c:IsAttribute(ATTRIBUTE_FIRE) and c:IsType(TYPE_SYNCHRO) and c:IsAbleToExtra()
end
