--御巫かみかくし
--Sibango Guardianwalk
--scripted by Naim
local s,id=GetID()
function s.initial_effect(c)
	--Equip 1 monster the opponent controls to a "Sibango" monster you control
	...
	c:RegisterEffect(e1)
	--Special Summon 1 "Sibango" monster from the hand or that is banished
...
function s.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>ft
		...
		and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsSetCard,SET_SIBANGO),tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,s.eqfilter,tp,0,LOCATION_MZONE,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
end
function s.ritmonfilter(c)
	return c:IsFaceup() and c:IsOriginalType(TYPE_MONSTER) and c:IsOriginalType(TYPE_LOCKED)
end
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	local eqc=Duel.SelectMatchingCard(tp,aux.FaceupFilter(Card.IsSetCard,SET_SIBANGO),tp,LOCATION_MZONE,0,1,1,nil):GetFirst()
	...
	if Duel.Equip(tp,tc,eqc) then
		...
		local ct=Duel.GetMatchingGroupCount(aux.FaceupFilter(Card.IsEquipActional),tp,LOCATION_ONFIELD,0,nil)
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_SIBANGO) and (c:IsFaceup() or c:IsLocation(LOCATION_HAND))
