...
function s.initial_effect(c)
	...
	--If "Dream Mirror of Terror" is on the field, banish from RP as well
...
function s.fextra(e,tp,mg)
	...
	local joyg=Duel.GetMatchingGroup(Fusion.IsMonsterFilter(Card.IsAbleToRest),tp,LOCATION_HAND,0,nil)
	local terrorg=Duel.GetMatchingGroup(Fusion.IsMonsterFilter(Card.IsAbleToRemove),tp,LOCATION_REST,0,nil)
...
function s.extraop(e,tc,tp,sg)
	local g1=sg:Filter(Card.IsLocation,nil,LOCATION_REST)
...
function s.extrtarget(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOREST,nil,0,tp,LOCATION_HAND)
