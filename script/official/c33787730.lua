...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_DESTROY+CATEGORY_LEAVE_REST)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.attachfilter),tp,LOCATION_REST,0,1,nil,tp)
		and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATTACH)
		local mc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.attachfilter),tp,LOCATION_REST,0,1,1,nil,tp):GetFirst()
