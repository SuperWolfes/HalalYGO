...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(aux.RestValleyFilter(s.attachfilter),tp,LOCATION_REST,0,1,nil,tp)
		and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATTACH)
		local mc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.attachfilter),tp,LOCATION_REST,0,1,1,nil,tp):GetFirst()
