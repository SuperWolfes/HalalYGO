...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Send 1 Fish "Shark" monster or 1 "Xyz" Actional/Trap from your Deck to the RP
	...
	c:RegisterEffect(e2)
	--Add 1 "Aqua Jet" card from your Deck or RP to your hand
...
s.listed_names={id}
s.listed_series={SET_SHARK,SET_XYZ,SET_AQUA_JET}
...
function s.spop(e,tp,eg,ep,ev,re,r,rp,c)
	--You cannot Special Summon "Jet Shark" for the rest of this turn
	local e1=Effect.CreateEffect(c)
...
function s.tgfilter(c)
	return ((c:IsRace(RACE_FISH) and c:IsSetCard(SET_SHARK)) or (c:IsSetCard(SET_XYZ) and c:IsActionalTrap())) and c:IsAbleToGrave()
end
...
function s.thfilter(c)
	return c:IsSetCard(SET_AQUA_JET) and c:IsAbleToHand()
end
