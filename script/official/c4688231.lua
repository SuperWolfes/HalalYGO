...
function s.rettg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.retfilter1,tp,LOCATION_REST,0,2,nil)
		and Duel.IsExistingTarget(Card.IsAbleToHand,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g1=Duel.SelectTarget(tp,s.retfilter1,tp,LOCATION_REST,0,2,2,nil)
...
function s.retop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=g:Filter(Card.IsLocation,nil,LOCATION_REST)
...
function s.spfilter(c,e,tp)
	...
	return c:IsSetCard(0xe1) and c:IsType(TYPE_PENDULUM) and (c:IsLocation(LOCATION_REST) or c:IsFaceup())
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),tp,loc,0,1,1,nil,e,tp)
	if #g>0 then
