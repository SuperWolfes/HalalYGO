...
function s.rfilter(c,e,tp,ft)
	return c:IsSetCard(0x88) and c:IsRace(RACE_BEASTWARRIOR) and (ft>0 or (c:GetSequence()<5 and c:IsControler(tp))) and (c:IsFaceup() or c:IsControler(tp))
		and Duel.IsExistingTarget(s.spfilter,tp,LOCATION_REST,0,1,nil,c:GetCode(),e,tp)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST) and s.spfilter(chkc,e:GetLabel(),e,tp) end
	...
	local sg=Duel.SelectTarget(tp,s.spfilter,tp,LOCATION_REST,0,1,1,nil,code,e,tp)
