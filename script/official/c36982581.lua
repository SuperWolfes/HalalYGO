...
function s.spfilter(c,e,tp)
	return c:IsLockedMonster() and c:IsSetCard(SET_GIOJIKI) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LOCKED,tp,false,true)
		and Duel.IsExistingMatchingCard(s.cfilter,tp,0,LOCATION_MZONE,1,nil,e,c)
end
function s.cfilter(c,e,sc)
	return c:IsFaceup() and c:IsCanBeLockedMaterial(sc) and not c:IsImmuneToEffect(e)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local rparams={filter=aux.FilterBoolFunction(Card.IsSetCard,SET_GIOJIKI),lvtype=RITPROC_EQUAL,stage2=s.stage2}
	local rittg=Locked.Target(rparams)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	local rparams={filter=aux.FilterBoolFunction(Card.IsSetCard,SET_GIOJIKI),lvtype=RITPROC_EQUAL,stage2=s.stage2}
	local rittg,ritop=Locked.Target(rparams),Locked.Operation(rparams)
	...
	if op==1 then
		...
		sc:SetMaterial(rg)
		Duel.ReleaseLockedMaterial(rg)
		Duel.BreakEffect()
		if Duel.SpecialSummon(sc,SUMMON_TYPE_LOCKED,tp,tp,false,true,POS_FACEUP)==0 then return end
...
function s.tdfilter(c)
	return c:IsSetCard(SET_GIOJIKI) and c:IsMonster() and c:IsAbleToDeck()
end
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp) and s.tdfilter(chkc) end
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToDeck() and Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_REST,0,1,nil) end
