--破壊剣士の揺籃
--Prologue of the Mismatching Swordsman
local s,id=GetID()
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Make "Mismatching Sword" cards you control unanble to be destroyed by battle or card effects
...
function s.cfilter(c)
	return ((c:IsSetCard(SET_MISMATCHING_SWORD) and not c:IsCode(id)) or (c:IsSetCard(SET_BUSTER_BLADER) and c:IsMonster()))
		and c:IsAbleToRestAsCost()
end
function s.rescon(sg,e,tp,mg)
	return sg:IsExists(Card.IsSetCard,1,nil,SET_MISMATCHING_SWORD)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA|LOCATION_REST,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA|LOCATION_REST)
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_EXTRA|LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
...
function s.immop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Your "Mismatching Sword" cards cannot be destroyed by battle or card effects
	local e1=Effect.CreateEffect(e:GetHandler())
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_INDESTRUCTABLE_EFFECT)
	e1:SetTargetRange(LOCATION_ONFIELD,0)
	e1:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,SET_MISMATCHING_SWORD))
	e1:SetValue(1)
