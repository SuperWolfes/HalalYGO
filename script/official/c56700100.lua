...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Add up to 3 "Purrely" Quick-Play Actionals to the hand
...
function s.qpfilter(c)
	return c:IsSetCard(SET_PURRELY) and c:IsActional() and c:IsType(TYPE_QUICKPLAY) and c:IsAbleToHand()
end
...
function s.rthop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.GetMatchingGroup(s.qpfilter,tp,LOCATION_REST,0,nil)
