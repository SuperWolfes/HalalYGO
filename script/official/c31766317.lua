...
function s.rfilter(c)
	return c:IsRace(RACE_TAINTED) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
end
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return #eg==1 and dc~=e:GetHandler() and dc:IsFaceup() and dc:IsLocation(LOCATION_MZONE) 
		and dc:IsRace(RACE_TAINTED) and dc:IsAttribute(ATTRIBUTE_DARK) and not dc:IsReason(REASON_REPLACE) 
		and Duel.IsExistingMatchingCard(s.rfilter,tp,LOCATION_MZONE+LOCATION_REST,0,1,nil) end
...
function s.filter(c,e,tp)
	return c:IsRace(RACE_TAINTED) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelAbove(6)
