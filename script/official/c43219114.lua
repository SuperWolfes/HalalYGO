...
function s.initial_effect(c)
	--Show up to 3 "Blue-Eyes White Dragon(s)" from your hand, face-up Monster Zone, and/or RP, then destroy that many cards your opponent controls
	...
	c:RegisterEffect(e1)
	--Locked Summon 1 Locked Monster from your hand, by Tributing "Blue-Eyes White Dragon(s)" from your hand or field
	local e2=Locked.CreateProc({handler=c,lvtype=RITPROC_EQUAL,matfilter=function(c) return c:IsCode(CARD_BLUEEYES_W_DRAGON) end,desc=aux.Stringid(id,1)})
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.showfilter,tp,LOCATION_HAND|LOCATION_MZONE|LOCATION_REST,0,1,nil)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local showg=Duel.GetMatchingGroup(s.showfilter,tp,LOCATION_HAND|LOCATION_MZONE|LOCATION_REST,0,nil)
