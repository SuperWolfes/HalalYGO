...
function s.initial_effect(c)
	...
	e3:SetCategory(CATEGORY_TOREST+CATEGORY_TOEXTRA)
...
function s.ritlimit(e,se,sp,st)
	if (st&SUMMON_TYPE_LOCKED)==SUMMON_TYPE_LOCKED then
...
function s.applytg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local b1=ex_ct>=2 and Duel.IsExistingMatchingCard(Card.IsAbleToRest,tp,0,LOCATION_HAND|LOCATION_EXTRA,ex_ct//2,nil)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOREST,nil,1,1-tp,LOCATION_HAND|LOCATION_EXTRA)
...
function s.applyop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 then
		...
		local g=Duel.SelectMatchingCard(1-tp,Card.IsAbleToRest,tp,0,LOCATION_HAND|LOCATION_EXTRA,send_ct,send_ct,nil)
		if #g>0 then
