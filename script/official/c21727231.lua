...
function s.eqfilter(c)
	return c:IsMonster() and (c:IsSetCard(0x17b) or c:IsRace(RACE_AQUA))
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST) and s.eqfilter(chkc) and not chkc:IsForbidden() end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingTarget(aux.AND(s.eqfilter,aux.NOT(Card.IsForbidden)),tp,LOCATION_REST,0,1,nil)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local g=Duel.SelectTarget(tp,aux.AND(s.eqfilter,aux.NOT(Card.IsForbidden)),tp,LOCATION_REST,0,1,1,nil)
...
function s.thfilter(c,e,tp)
	return c:IsCanBeEffectTarget(e) and c:IsAbleToHand() and (c:IsControler(1-tp)
		or (c:GetSequence()<5 and c:IsFaceup() and c:IsSetCard(0x17b)))
end
