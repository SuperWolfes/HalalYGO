...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return lp>0 and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_DECK+LOCATION_REST,0,1,nil,e,tp,Duel.GetLocationCount(tp,LOCATION_MZONE),lp)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.filter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil,e,tp,Duel.GetLocationCount(tp,LOCATION_MZONE),lp):GetFirst()
	if tc then
