...
function s.initial_effect(c)
	--Send 1 Level 7 or higher Normal Monster from your hand or Deck to the RP, and if you do, Special Summon this card
	...
	c:RegisterEffect(e1)
	--Special Summon 1 Normal Monster from your RP in Defense Position
...
function s.tgfilter(c)
	return c:IsType(TYPE_NORMAL) and c:IsLevelAbove(7) and c:IsAbleToRest()
end
...
function s.gysptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(s.gyspfilter,tp,LOCATION_REST,0,1,c,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.gyspfilter,tp,LOCATION_REST,0,1,1,c,e,tp)
