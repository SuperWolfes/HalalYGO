...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add 1 Actional from the RP to the hand
...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsReason(REASON_DESTROY) and c:IsPreviousLocation(LOCATION_ONFIELD)
		...
		and Duel.IsExistingMatchingCard(Card.IsRace,tp,LOCATION_REST,0,3,nil,RACE_MENTOR)
end
function s.filter(c)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(Card.IsRace,tp,LOCATION_REST,0,3,nil,RACE_MENTOR) then return end
