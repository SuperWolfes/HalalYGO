...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Return 1 "Amazonian" monster from your RP to the hand or Deck
...
function s.spcostfilter(c)
	return c:IsSetCard(SET_AMAZONIAN) and c:IsMonster() and not c:IsPublic()
end
...
function s.thfilter(c)
	return c:IsSetCard(SET_AMAZONIAN) and c:IsMonster() and not c:IsCode(id) and (c:IsAbleToHand() or c:IsAbleToDeck())
end
