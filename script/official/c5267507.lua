...
function s.initial_effect(c)
	...
	--Send 1 "Earthbound Immortal" monster or "Red Dragon Archtainted" from your hand or face-up Monster Zone to the RP
...
s.listed_names={CARD_RED_DRAGON_ARCHTAINTED,97489701} --"Red Nova Dragon"
s.listed_series={SET_EARTHBOUND,SET_EARTHBOUND_IMMORTAL}
function s.tgfilter(c)
	return (c:IsSetCard(SET_EARTHBOUND_IMMORTAL) or c:IsCode(CARD_RED_DRAGON_ARCHTAINTED)) and c:IsMonster() and c:IsAbleToGrave()
