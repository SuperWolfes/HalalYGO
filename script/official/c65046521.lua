...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Place "Interwoven" Actional/Trap
...
function s.filter(c,e,tp)
	return  c:IsRace(RACE_MENTOR) and c:IsDefense(1500)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,0,LOCATION_HAND+LOCATION_REST)
end
...
function s.tffilter(c,tp)
	return c:IsActionalTrap() and c:IsType(TYPE_CONTINUOUS) and c:IsSetCard(0x0c0) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
end
function s.tftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(s.tffilter,tp,LOCATION_REST,0,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local g=Duel.SelectTarget(tp,s.tffilter,tp,LOCATION_REST,0,1,1,e:GetHandler(),tp)
