...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsFaceup() and not tc:IsImmuneToEffect(e) then
		...
		--Destroy cards in the Actional/Trap Zone and equip 5 "Forbidden One" monsters to this card
...
function s.eqfilter(c,tp)
	return c:IsSetCard(SET_UNLIKED_ONE) and c:IsMonster() and c:CheckUniqueOnField(tp) and not c:IsForbidden()
end
