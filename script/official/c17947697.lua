...
function s.initial_effect(c)
	--Place 1 "True Light" from your hand, Deck, or RP face-up in your Actional & Trap Zone
	...
	c:RegisterEffect(e2)
	--Special Summon 1 "Blue-Eyes White Dragon" or 1 Level 1 LIGHT Tuner from your RP
...
function s.plfilter(c,tp)
	return c:IsCode(62089826) and not c:IsUnliked() and c:CheckUniqueOnField(tp)
end
function s.pltg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 
		and Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,nil,tp) end
end
function s.plop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.plfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,1,nil,tp):GetFirst()
	if tc then 
