...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,c)
	...
	local rg=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,true,nil,tp)
	if rg and #rg>0 then
		rg:KeepAlive()
		e:SetLabelObject(rg)
		return true
...
function s.tdfilter(c,tp)
	return (c:IsLevel(8) or c:IsRank(8)) and (c:IsAbleToDeck() or c:IsAbleToExtra())
		and ((c:IsType(TYPE_LOCKED|TYPE_FUSION) and Duel.IsPlayerCanDraw(tp,1)) or c:IsType(TYPE_SYNCHRO|TYPE_XYZ))
end
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST) and s.tdfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_REST,0,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local tc=Duel.SelectTarget(tp,s.tdfilter,tp,LOCATION_REST,0,1,1,nil,tp):GetFirst()
	Duel.SetOperationInfo(0,CATEGORY_TODECK,tc,1,tp,LOCATION_REST)
...
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsType(TYPE_LOCKED|TYPE_FUSION) and tc:IsLocation(LOCATION_DECK+LOCATION_EXTRA) then
		Duel.BreakEffect()
