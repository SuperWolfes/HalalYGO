...
function s.initial_effect(c)
	...
	e1:SetRange(LOCATION_DECK+LOCATION_REST)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) or Duel.GetLocationCount(tp,LOCATION_MZONE)<2
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,1800,1300,5,RACE_WANDERER,ATTRIBUTE_DARK)
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id+2,0,TYPES_TOKEN,1200,1200,3,RACE_WANDERER,ATTRIBUTE_DARK) then
		return
	end
	local c=e:GetHandler()
