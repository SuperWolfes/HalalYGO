...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- EARTH monsters become "Vernusip" monsters
	...
	e2:SetValue(SET_VERNUSIP)
	c:RegisterEffect(e2)
	-- "Vernusip" cost replacement
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(id,0))
	e3:SetType(EFFECT_TYPE_FIELD)
	e3:SetProperty(EFFECT_FLAG_PLAYER_TARGET)
	e3:SetCode(CARD_VERNUSIP_COROLLA)
...
function s.repval(base,e,tp,eg,ep,ev,re,r,rp,chk,extracon)
	...
	return c:IsMonster() and c:IsSetCard(SET_VERNUSIP) and c:IsDiscardable()
end
function s.repop(base,e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_CARD,0,id)
	local c=base:GetHandler()
	if #{Duel.GetPlayerEffect(tp,CARD_VERNUSIP_COROLLA)}>1 then
