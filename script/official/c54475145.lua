...
function s.initial_effect(c)
	--Add 1 "Mileon Ankh" from your Deck to your hand
	...
	c:RegisterEffect(e4)
	--Register that a player has activated "Mileon Ankh" during this Duel
...
s.listed_series={SET_MILEON}
s.listed_names={CARD_MILEON_CROSS}
function s.regop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.HasFlagEffect(rp,id) and re:GetHandler():IsCode(CARD_MILEON_CROSS) and re:IsHasType(EFFECT_TYPE_ACTIVATE) then
...
function s.thfilter(c)
	return c:IsCode(CARD_MILEON_CROSS) and c:IsAbleToHand()
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 and g:IsExists(Card.IsLocation,1,nil,LOCATION_REST)
