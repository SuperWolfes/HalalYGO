...
function s.thfilter(c)
	return c:IsSetCard(SET_RECIPE) and c:IsLockedActional() and not c:IsCode(id) and c:IsAbleToHand()
end
function s.stage2(mat,e,tp,eg,ep,ev,re,r,rp,tc)
	if not tc:IsCode(26223582) then return end
	local g=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.thfilter),tp,LOCATION_REST|LOCATION_DECK,0,nil)
