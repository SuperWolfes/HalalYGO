...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Draw cards equal to the number of S/T sent to the RP
...
function s.cfilter(c)
	return c:IsRace(RACE_MENTOR) and c:IsLevelAbove(6) and c:IsAbleToRestAsCost()
end
...
function s.spfilter(c,e,tp)
	return c:IsCode(CARD_DARK_MENTOR,CARD_DARK_MENTOR_GIRL) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif op==2 and c:IsRelateToEffect(e) and Duel.SendtoRest(c,REASON_EFFECT)>0 and c:IsLocation(LOCATION_REST)
