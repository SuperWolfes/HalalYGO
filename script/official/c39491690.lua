...
function s.initial_effect(c)
	--Return cards on the field to the hand up to the number of Fusion and Synchro Monsters (Tainted, Illusion, and Mentor) you control and in your RP
...
function s.thctfilter(c)
	return c:IsType(TYPE_FUSION|TYPE_SYNCHRO) and c:IsRace(RACE_TAINTED|RACE_ILLUSION|RACE_MENTOR) and c:IsFaceup()
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsOnField() and chkc:IsAbleToHand() and chkc~=c end
	local ct=Duel.GetMatchingGroupCount(s.thctfilter,tp,LOCATION_MZONE|LOCATION_REST,0,nil)
