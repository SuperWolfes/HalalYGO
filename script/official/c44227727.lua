...
function s.initial_effect(c)
	...
	c:RegisterEffect(Fusion.CreateSummonEff(c,aux.FilterBoolFunction(Card.IsRace,RACE_TAINTED)))
...
function s.eqfilter(c,tc)
	return not c:IsUnliked() and ((c:IsCode(80621422) and c:CheckEquipTarget(tc)) or (c:IsMonster() and c:IsSetCard(0x13f)))
end
