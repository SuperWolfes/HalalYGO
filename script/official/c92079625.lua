...
function s.initial_effect(c)
	--Send 1 "Shaddoll" monster from your Extra Deck to the RP and banish 1 opponent's face-up monster
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_TOREST+CATEGORY_REMOVE)
	...
	c:RegisterEffect(e1)
	--Send cards from the top of your Deck to the RP, equal to the number of different original Attributes among the monsters on the field
...
function s.tgfilter(c,att)
	return c:IsSetCard(SET_SHADDOLL) and c:IsAbleToRest() and c:IsAttribute(att)
end
