...
function s.initial_effect(c)
	...
	s.locked_matching_function[c]=aux.FilterEqualFunction(Card.IsSetCard,0xb4)
end
s.listed_series={0xb4}
function s.spfilter(c,e,tp,mc)
	return c:IsSetCard(0xb4) and c:IsLockedMonster() and (not c.locked_custom_check or c.locked_custom_check(e,tp,Group.FromCards(mc),c))
		and (not c.mat_filter or c.mat_filter(mc,tp)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LOCKED,tp,false,true)
		and mc:IsCanBeLockedMaterial(c)
end
function s.rfilter(c,mc)
	local mlv=mc:GetLockedLevel(c)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if b1 and (not b2 or Duel.SelectYesNo(tp,aux.Stringid(id,0))) then
		...
		tc=tg:GetFirst()
		for tc in aux.Next(tg) do
			Duel.SpecialSummonStep(tc,SUMMON_TYPE_LOCKED,tp,tp,false,true,POS_FACEUP)
			tc:CompleteProcedure()
		end
