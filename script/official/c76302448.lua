...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_LIGHT_BARRIER),tp,LOCATION_FZONE,0,1,nil) then
		...
		local b2=Duel.IsExistingMatchingCard(aux.RestValleyFilter(s.tailsspfilter),tp,LOCATION_REST,0,1,nil,e,tp)
	...
	elseif coin==COIN_TAILS then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.tailsspfilter),tp,LOCATION_REST,0,1,1,nil,e,tp)
		if #g>0 then
			Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
		end
	end
