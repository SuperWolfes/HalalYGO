...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Add 1 card from your RP to your hand that has a coin tossing effect
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		or Duel.IsExistingMatchingCard(s.tailsspfilter,tp,LOCATION_REST,0,1,nil,e,tp)) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_LIGHT_BARRIER),tp,LOCATION_FZONE,0,1,nil) then
		...
		local b2=Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.tailsspfilter),tp,LOCATION_REST,0,1,nil,e,tp)
	...
	elseif coin==COIN_TAILS then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.tailsspfilter),tp,LOCATION_REST,0,1,1,nil,e,tp)
		if #g>0 then
			Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
		end
	end
