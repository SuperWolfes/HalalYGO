...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Special Summon 1 monster from your opponent's RP to your field
...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=Duel.GetMatchingGroupCount(Card.IsRace,tp,LOCATION_REST,0,nil,RACE_TOXIC)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 then
		--This card gains 800 ATK for each Toxic monster in your RP
		c:UpdateAttack(ct*800,RESETS_STANDARD_DISABLE)
	elseif op==2 then
		--Inflict 500 damage to your opponent for each Toxic monster in your RP
		Duel.Damage(1-tp,ct*500,REASON_EFFECT)
	end
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(Card.IsCanBeSpecialSummoned,tp,0,LOCATION_REST,1,nil,e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_REST)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,Card.IsCanBeSpecialSummoned,tp,0,LOCATION_REST,1,1,nil,e,0,tp,false,false)
	if #g>0 then
