...
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Link Summon procedure: 2 Toxic monsters
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsRace,RACE_TOXIC),2,2)
	--Special Summon 1 Level 5 or higher Toxic monster from your RP or banishment
...
function s.spfilter(c,e,tp)
	return c:IsLevelAbove(5) and c:IsRace(RACE_TOXIC) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsFaceup()
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Treat this card as a Continuous Actional
	local e1=Effect.CreateEffect(c)
...
function s.destgfilter(c,tp)
	return c:IsLevelAbove(5) and c:IsRace(RACE_TOXIC) and c:IsFaceup()
