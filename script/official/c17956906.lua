...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local g=Duel.GetMatchingGroup(s.filter1,tp,LOCATION_REST,0,nil,e,tp)
	...
	local n = Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) and 1 or math.min(fg,ft)
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,sg,#sg,tp,LOCATION_REST)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if count>0 then
		local g2,ft2=Duel.GetMatchingGroup(s.filter2,tp,0,LOCATION_REST,nil,e,1-tp),math.min(Duel.GetLocationCount(1-tp,LOCATION_MZONE),count)
