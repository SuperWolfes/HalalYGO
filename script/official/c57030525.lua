...
function s.initial_effect(c)
	--This card's name becomes "Armed Dragon LV3" while on the field or in the RP
...
function s.spcostfilter(c,e,tp)
	return c:IsMonster() and c:IsAbleToRestAsCost() and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND|LOCATION_DECK,0,1,c,e,tp)
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsAbleToRest() and Duel.GetMZoneCount(tp,c)>0 end
	Duel.SetOperationInfo(0,CATEGORY_TOREST,c,1,tp,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND|LOCATION_DECK)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SendtoRest(c,REASON_EFFECT)>0 and c:IsLocation(LOCATION_REST)
