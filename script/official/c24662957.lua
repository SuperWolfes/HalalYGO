...
s.listed_names={CARD_RED_DRAGON_ARCHTAINTED,50078509} --Taintedish Chain
function s.rmfilter(c)
	return c:IsFaceup() and c:IsAttackAbove(2000) and c:IsAbleToRemove()
end
function s.rdafilter(c)
	return c:IsFaceup() and (c:IsCode(CARD_RED_DRAGON_ARCHTAINTED)
		or (c:IsType(TYPE_SYNCHRO) and c:IsMonster() and c:ListsCode(CARD_RED_DRAGON_ARCHTAINTED)))
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if ret_eff and e:GetLabel()>0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.setfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		--Set 1 "Taintedish Chain" from the Deck or RP
