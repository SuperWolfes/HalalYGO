...
function s.initial_effect(c)
	--Send 1 "Xyz" card to the RP or add it to the hand if an Xyz monster with an Xyz monster as material is on the field
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_TOREST+CATEGORY_SEARCH+CATEGORY_TOHAND)
...
function s.cfilter(c,to_hand)
	return c:IsSetCard(SET_XYZ) and not c:IsCode(id) and (c:IsAbleToRest() or (to_hand and c:IsAbleToHand()))
end
...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	local to_hand=Duel.IsExistingMatchingCard(s.xyzfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil)
	local hintmsg=to_hand and aux.Stringid(id,2) or HINTMSG_TOREST
...
function s.detachop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and sc:IsControler(tp) and sc:IsMonster()
		and sc:IsType(TYPE_XYZ) and sc:IsLocation(LOCATION_REMOVED|LOCATION_REST)
