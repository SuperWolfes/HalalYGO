...
function s.plfilter(c,tp)
	return c:IsCode(id) and not c:IsUnliked() and c:CheckUniqueOnField(tp)
end
function s.thcostfilter(c)
	return c:IsCode(id) and c:IsAbleToRestAsCost() and c:IsFaceup()
end
function s.thfilter(c)
	return c:IsSetCard(SET_UNLIKED_ONE) and c:IsMonster() and c:IsAbleToHand()
end
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local b1=Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(s.plfilter,tp,LOCATION_DECK|LOCATION_REST,0,1,nil,tp)
	local b2=c:IsAbleToRestAsCost() and c:IsStatus(STATUS_EFFECT_ENABLED)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		--Place 1 "Break the Seal" from your Deck or RP face-up on your field
		if Duel.GetLocationCount(tp,LOCATION_SZONE)<=0 then return end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
		local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.plfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,1,nil,tp):GetFirst()
		if sc then
			Duel.MoveToField(sc,tp,tp,LOCATION_SZONE,POS_FACEUP,true)
		end
	elseif op==2 then
		--Add 1 "Unliked One" monster from your Deck to your hand
...
function s.rthcostfilter(c)
	return c:IsSetCard(SET_UNLIKED_ONE) and c:IsMonster() and not c:IsPublic()
end
