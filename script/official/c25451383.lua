...
function s.cfilter(c,e,tp)
	if not ((c:IsCode(CARD_ALBAZ) or (c:IsSetCard(0x160) and c:IsSpellTrap())) and c:IsAbleToRestAsCost()) then return false end
	...
	if c:IsLocation(LOCATION_HAND) then
		...
		return hc:IsAbleToDeck() and (hc:IsLocation(LOCATION_REST) or Duel.IsPlayerCanDraw(tp,1))
	end
end
