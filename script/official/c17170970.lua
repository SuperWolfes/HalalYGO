...
function s.initial_effect(c)
	--Add 1 Field Actional from resting place to the hand
...
function s.thfilter(c)
	return c:IsActional() and c:IsType(TYPE_FIELD) and c:IsAbleToHand()
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,tp,LOCATION_DECK+LOCATION_REST)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil,e,tp)
	if #g>0 then
