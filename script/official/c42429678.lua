...
function s.tgfilter(c)
	return c:IsMonster() and c:IsSetCard(0x135) and c:IsAbleToRest()
end
...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsLocation(LOCATION_REST) and r==REASON_SYNCHRO and c:GetReasonCard():IsRace(RACE_CYBERSE)
end
function s.spfilter(c,e,tp)
	return not c:IsCode(id) and c:IsLocation(LOCATION_REST) and c:IsControler(tp) and c:IsCanBeEffectTarget(e)
