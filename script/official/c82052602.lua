...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in aux.Next(eg) do
		...
		if tc:IsSetCard(0x54) and tc:IsLocation(LOCATION_REST) and tc:IsReason(REASON_BATTLE)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
		and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_REST,0,1,nil,Duel.GetTurnCount(),e,tp) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_REST,0,ft,ft,nil,Duel.GetTurnCount(),e,tp)
	if #g>0 then
