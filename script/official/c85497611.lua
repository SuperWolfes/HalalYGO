...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special Ssummon 1 "Odd-Eyes" from your Deck or RP
...
function s.gyfilter(c,tp,mzone_chk)
	return c:IsCode(53025096) and (c:IsFaceup() or not c:IsLocation(LOCATION_MZONE)) and c:IsAbleToRest()
...
function s.sptg1(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOREST,nil,1,tp,LOCATION_DECK+LOCATION_HAND+LOCATION_MZONE)
...
function s.spop1(e,tp,eg,ep,ev,re,r,rp)
	...
	if gc and Duel.SendtoRest(gc,REASON_EFFECT)>0 and gc:IsLocation(LOCATION_REST) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
