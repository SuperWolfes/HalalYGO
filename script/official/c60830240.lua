...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Return up to 11 banished "Infernoid" monsters to the RP
...
function s.dccostfilter(c)
	return not c:IsPublic()
		...
		or (c:IsSetCard(SET_VOID) and c:IsActionalTrap()))
end
...
function s.dcop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=Duel.SendtoRest(g,REASON_EFFECT|REASON_DISCARD)
...
function s.tgcon(e,tp,eg,ep,ev,re,r,rp)
	return aux.exccon(e,tp,eg,ep,ev,re,r,rp)
		and not Duel.IsExistingMatchingCard(aux.NOT(aux.FaceupFilter(Card.IsRace,RACE_TAINTED)),tp,LOCATION_MZONE,0,1,nil)
end
