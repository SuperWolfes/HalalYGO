...
function s.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.GraveValleyFilter(s.filter2),tp,LOCATION_HAND+LOCATION_REST,0,1,nil,tp) end
end
function s.activate2(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOFIELD)
	local tc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.filter2),tp,LOCATION_HAND+LOCATION_REST,0,1,1,nil,tp):GetFirst()
