...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Destroy monsters on the field, banish from the RP and special summon itself
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,tp,LOCATION_REST)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(s.rmfilter,tp,LOCATION_MZONE+LOCATION_REST,0,c)
	if #g>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and c:IsLocation(LOCATION_REST) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
