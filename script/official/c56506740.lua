...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Normal Monsters and "Primite" monsters you control gain 300 ATK for each Normal Monster in your RP with different names
	...
	e3:SetValue(function(e,c) return 300*Duel.GetMatchingGroup(Card.IsType,e:GetHandlerPlayer(),LOCATION_REST,0,nil,TYPE_NORMAL):GetClassCount(Card.GetCode) end)
	c:RegisterEffect(e3)
end
s.listed_names={id}
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,1,1,nil,e,tp,code)
	if #g>0 then
