...
function s.initial_effect(c)
	...
	c:SetCounterLimit(COUNTER_ACTIONAL,9)
	c:EnableReviveLimit()
	--2 Level 8 monsters
	Xyz.AddProcedure(c,nil,8,2)
	--Place Actional Counter on this card
	...
	c:RegisterEffect(e1)
	--Search 1 Actional or 1 Mentor monster, OR Special Summon 1 Mentor monster
...
function s.acop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ep==1-tp and not re:IsActionalEffect() and c:HasFlagEffect(1) then
...
function s.thfilter(c)
	return (c:IsActional() or (c:IsRace(RACE_MENTOR) and c:IsType(TYPE_EFFECT))) and c:IsAbleToHand()
end
