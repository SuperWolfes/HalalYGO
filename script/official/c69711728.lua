...
function s.cfilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_FUSION) and c:IsSummonType(SUMMON_TYPE_FUSION)
		and Duel.IsExistingTarget(s.filter,tp,LOCATION_REST,0,1,nil,c:GetLevel(),e,tp)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST) and s.filter(chkc,eg:GetFirst():GetLevel(),e,tp) end
	if chk==0 then return eg:IsExists(s.cfilter,1,nil,e,tp)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.filter,tp,LOCATION_REST,0,1,1,nil,eg:GetFirst():GetLevel(),e,tp)
