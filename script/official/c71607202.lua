...
function s.initial_effect(c)
	...
	e0:SetValue(1)
	c:RegisterEffect(e0)
	--Mismatching replacement by Tributing 1 Tainted monster
	...
	c:RegisterEffect(e1)
	--Special Summon 1 Tainted monster from your RP
...
function s.trbfilter(c,e)
	return c:IsRace(RACE_TAINTED) and c:IsReleasableByEffect() and not c:IsImmuneToEffect(e)
...
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_TAINTED) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Cannot Special Summon monsters, except Tainted monsters
	...
	e1:SetTarget(function(e,c) return not c:IsRace(RACE_TAINTED) end)
