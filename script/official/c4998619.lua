...
function s.tgfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xe3) and c:IsAbleToRestAsCost()
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local g=nil
	local rg=Duel.GetMatchingGroup(s.tgfilter,tp,LOCATION_MZONE,0,nil)
	local g=aux.SelectUnselectGroup(rg,e,tp,3,3,aux.ChkfMMZ(1),1,tp,HINTMSG_TOREST,nil,nil,true)
	if #g>0 then
		g:KeepAlive()
		e:SetLabelObject(g)
		return true
...
function s.sptg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_REST+LOCATION_DECK)
end
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)~=0 then
		local sg=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.thfilter),tp,LOCATION_DECK+LOCATION_REST,0,nil)
