...
function s.tffilter(c,tp)
	return c:IsActionalTrap() and c:IsSetCard(0x109) and not c:IsForbidden() and c:CheckUniqueOnField(tp)
...
function s.tfop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.tffilter),tp,LOCATION_HAND+LOCATION_REST+LOCATION_DECK,0,1,1,nil,tp):GetFirst()
	if tc then
