...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Place 1 "Monster Rerest" on top of your Deck
...
s.listed_names={CARD_MONSTER_REREST,10000020}
...
function s.tdfilter(c,deck_count)
	if not c:IsCode(CARD_MONSTER_REREST) then return false end
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_DECK+LOCATION_REST,0,1,nil,deck_count) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,1,tp,LOCATION_REST+LOCATION_DECK)
