--Ｅｍボール・ライダー
--Performanced Ball Balancer
--scripted by Naim
local s,id=GetID()
function s.initial_effect(c)
	Pendulum.AddProcedure(c)
	--Send 1 face-up "Performanced" Pendulum Monster from your Extra Deck to the RP
	...
	e2:SetCondition(function(e,tp) return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsSetCard,SET_PERFORMANCED),tp,LOCATION_MZONE,0,1,nil) end)
	e2:SetTarget(s.sptg)
...
function s.tgfilter(c)
	return c:IsSetCard(SET_PERFORMANCED) and c:IsType(TYPE_PENDULUM) and c:IsFaceup() and c:IsAbleToGrave()
end
...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_REST) and tc:GetLeftScale()>0
...
function s.tedfilter(c)
	return c:IsSetCard(SET_PERFORMANCED) and c:IsType(TYPE_PENDULUM)
end
function s.desfilter(c)
	return c:IsSetCard(SET_PERFORMANCED) and c:IsMonsterCard() and c:IsFaceup()
end
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		--Add 1 "Performanced" Pendulum Monster from your Deck to your Extra Deck face-up
	...
	elseif op==2 then
		--Destroy 1 "Performanced" Monster Card you control
