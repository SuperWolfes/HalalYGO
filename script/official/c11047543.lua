...
function s.filter1(c,e,tp)
	return c:IsFaceup() and c:IsRace(RACE_MENTAL) and c:IsType(TYPE_TUNER)
		and Duel.IsExistingTarget(s.filter2,tp,LOCATION_REMOVED,0,1,nil,e,tp,c:GetLevel())
end
function s.filter2(c,e,tp,lv)
	local clv=c:GetLevel()
	return clv>0 and c:IsFaceup() and c:IsRace(RACE_MENTAL) and not c:IsType(TYPE_TUNER)
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp,lv+clv)
end
function s.spfilter(c,e,tp,lv)
	return c:IsRace(RACE_MENTAL) and c:IsType(TYPE_SYNCHRO) and c:GetLevel()==lv and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
