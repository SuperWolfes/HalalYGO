...
function s.initial_effect(c)
	c:EnableAwakeLimit()
	--Make the opponent send 1 Actional/Trap to the RP
	...
	c:RegisterEffect(e2)
	--Shuffle 1 Locked Monster from your RP into the Deck and destroy 1 card the opponent controls
...
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOREST,nil,1,0,LOCATION_SZONE)
end
...
function s.cfilter(c,tp)
	return c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsSetCard(SET_CYBER_ANGEL) and c:IsLockedMonster()
end
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,1,nil)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.tdfilter),tp,LOCATION_REST,0,1,1,nil)
