...
function s.initial_effect(c)
	--Return 1 Illusion or Mentor to the hand or Extra Deck
...
function s.rtfilter(c,tp)
	return c:IsRace(RACE_ILLUSION|RACE_MENTOR) and c:IsFaceup() and Duel.GetMZoneCount(tp,c)>0
		and (c:IsAbleToExtra() or c:IsAbleToHand())
end
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_ILLUSION|RACE_MENTOR) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
