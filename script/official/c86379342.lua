...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Negate the effects of a Actional/Trap card
...
function s.lkfilter(c,e,tp)
	return c:IsType(TYPE_LINK) and c:IsFaceup() and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_REST,0,1,nil,e,tp,c:GetRace())
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),tp,LOCATION_HAND+LOCATION_REST,0,1,1,nil,e,tp,tc:GetRace())
	if #g>0 then
...
function s.negtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(1-tp) and chkc:IsNegatableActionalTrap() end
	if chk==0 then return Duel.IsExistingTarget(Card.IsNegatableActionalTrap,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_NEGATE)
	Duel.SelectTarget(tp,Card.IsNegatableActionalTrap,tp,0,LOCATION_ONFIELD,1,1,nil)
end
