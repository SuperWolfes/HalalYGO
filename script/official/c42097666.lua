--青き眼の精霊
--Guardian with Eyes of Blue
--scripted by pyrQ
local s,id=GetID()
function s.initial_effect(c)
	c:EnableAwakeLimit()
	--Link Summon procedure: 1 Level 4 or lower Dragon or Mentor monster
	...
	c:RegisterEffect(e1)
	--Add to your hand or send to the RP 1 "Mausoleum of White" from your Deck
	...
	c:RegisterEffect(e2)
	--Special Summon 1 "Blue-Eyes" monster from your hand or RP
...
function s.matfilter(c,lc,sumtype,tp)
	return c:IsLevelBelow(4) and c:IsRace(RACE_DRAGON|RACE_MENTOR,lc,sumtype,tp)
end
function s.thtgfilter(c)
	return c:IsCode(24382602) and (c:IsAbleToHand() or c:IsAbleToRest())
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
	if sc and Duel.SpecialSummonStep(sc,0,tp,tp,false,false,POS_FACEUP)
		and sc:IsType(TYPE_EFFECT) and sc:IsSummonLocation(LOCATION_REST) then
