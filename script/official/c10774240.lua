...
function s.initial_effect(c)
	c:EnableAwakeLimit()
	--Gains ATK while "Lo, the Prayers of the Voiceless Voice" is on your field or in your RP
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
	e1:SetCode(EFFECT_UPDATE_ATTACK)
	e1:SetRange(LOCATION_MZONE)
	e1:SetCondition(function(e) return Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,25801745),e:GetHandlerPlayer(),LOCATION_ONFIELD|LOCATION_REST,0,1,nil) end)
	e1:SetValue(2050)
	c:RegisterEffect(e1)
	--Search 1 "Voiceless Voice" monster, or 1 Warrior or Dragon Locked Monster
...
function s.thfilter(c)
	return ((c:IsSetCard(SET_VOICELESS_VOICE) and c:IsMonster()) or (c:IsRace(RACE_WARRIOR|RACE_DRAGON) and c:IsLockedMonster())) and c:IsAbleToHand()
end
