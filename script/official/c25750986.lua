...
function s.initial_effect(c)
	--Send 1 LIGHT and 1 DARK monster from deck to RP
...
function s.aclimit(e,tp,eg,ep,ev,re,r,rp)
	if ep~=tp or not (re:GetActivateLocation()==LOCATION_REST and re:IsActiveType(TYPE_MONSTER)) then return end
...
function s.elimit(e,te,tp)
	return te:GetActivateLocation()==LOCATION_REST and te:IsActiveType(TYPE_MONSTER)
end
