...
function s.tgfilter(c)
	return c:IsFaceup() and c:IsSetCard(SET_VANQUISH_MISS) and c:IsCanChangePosition()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.ChangePosition(tc,POS_FACEUP_DEFENSE,POS_FACEDOWN_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)>0 then
		local ct=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsSetCard,SET_VANQUISH_MISS),tp,LOCATION_MZONE,0,nil):GetClassCount(Card.GetCode)
