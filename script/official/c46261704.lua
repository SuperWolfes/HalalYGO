...
function s.initial_effect(c)
	--Special Summon 1 "Galaxy-Eyes Photon Dragon"
	...
	c:RegisterEffect(e1)
	--Mismatching replacement for "Photon" or "Galaxy" monsters
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 then
		...
		if Duel.ChangeAttackTarget(g:GetFirst()) and c:HasFlagEffect(id) and c:IsLocation(LOCATION_REST) and aux.nvfilter(c)
...
function s.repfilter(c,tp)
	return c:IsFaceup() and (c:IsSetCard(SET_PHOTON) or c:IsSetCard(SET_GALAXY))
		and c:IsLocation(LOCATION_MZONE) and c:IsControler(tp) and not c:IsReason(REASON_REPLACE)
