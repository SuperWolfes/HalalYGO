...
function s.mtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return #g>0 and Duel.GetFlagEffect(tp,id)==0 and Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_REST+LOCATION_EXTRA,0,1,nil,TYPE_XYZ) end
...
function s.mop(e,tp,eg,ep,ev,re,r,rp)
	...
	if e:GetHandler():IsRelateToEffect(e) and tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
		...
		local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(Card.IsType),tp,LOCATION_REST+LOCATION_EXTRA,0,1,1,tc,TYPE_XYZ)
		if #g>0 then
