--守護神-ネフティス
--Nepolonis, the Clean Preserver
--Scripted by Eerie Code
local s,id=GetID()
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Link Summon Procedure
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,SET_NEPOLONIS),2,2)
	--Activate 1 effect
...
function s.desfilter(c,e,tp,g,nc)
	local f=s.spfilter
	if nc then f=aux.RestValleyFilter(f) end
	return c:IsFaceup() and c:IsSetCard(SET_NEPOLONIS) and g:IsContains(c)
		and Duel.IsExistingMatchingCard(f,tp,LOCATION_REST,0,1,nil,e,tp,c)
end
function s.spfilter(c,e,tp,dc)
	return c:IsSetCard(SET_NEPOLONIS) and not c:IsOriginalCode(dc:GetOriginalCode())
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	--Search 1 Level 8 Winged Beast monster, then you can add 1 Locked Actional from your RP to your hand
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,s.thfilter1,tp,LOCATION_DECK,0,1,1,nil)
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		Duel.ConfirmCards(1-tp,g)
		local g2=Duel.GetMatchingGroup(aux.RestValleyFilter(s.thfilter2),tp,LOCATION_REST,0,nil)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	--Destroy 1 "Nepolonis" monster and Special Summon 1 "Nepolonis" monster with a different name
	...
	if dc and Duel.Destroy(dc,REASON_EFFECT)>0 then
		...
		local tc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.spfilter),tp,LOCATION_REST,0,1,1,nil,e,tp,dc):GetFirst()
