...
function s.thfilter2(c)
	return c:IsActional() and c:IsType(TYPE_LOCKED) and c:IsAbleToHand()
end
function s.desfilter(c,e,tp,g,nc)
	local f=s.spfilter
	if nc then f=aux.GraveValleyFilter(f) end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoHand(g,nil,REASON_EFFECT)>0 then
		...
		local g2=Duel.GetMatchingGroup(aux.GraveValleyFilter(s.thfilter2),tp,LOCATION_REST,0,nil)
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if dc and Duel.Destroy(dc,REASON_EFFECT)>0 then
		...
		local tc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),tp,LOCATION_REST,0,1,1,nil,e,tp,dc):GetFirst()
