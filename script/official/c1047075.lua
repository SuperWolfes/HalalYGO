...
function s.initial_effect(c)
	--Search 1 Actional/Trap that mentions "Flame Swordsman"
	...
	c:RegisterEffect(e2)
	--Send 1 "Flame Swordsman" or 1 monster that mentions it to the resting place
...
function s.thfilter(c)
	return c:IsActionalTrap() and c:ListsCode(CARD_FLAME_SWORDSMAN) and c:IsAbleToHand()
end
...
function s.tgfilter(c)
	return (c:IsCode(CARD_FLAME_SWORDSMAN) or (c:IsMonster() and c:ListsCode(CARD_FLAME_SWORDSMAN))) and not c:IsCode(id) and c:IsAbleToRest()
end
