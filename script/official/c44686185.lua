...
function s.initial_effect(c)
	--Special Summon 1 "Six Samurai" monster from the RP
	...
	c:RegisterEffect(e1)
	--Mismatching replacement for exactly 1 "Six Samurai" monster
...
function s.cfilter(c)
	return c:IsSetCard(SET_SIX_SAMURAI) and c:IsMonster() and c:IsAbleToRemoveAsCost()
		and (c:IsLocation(LOCATION_REST) or c:IsFaceup()) and aux.SpElimFilter(c,true,true)
end
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_SIX_SAMURAI) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.rescon(sg,e,tp,mg)
	return aux.ChkfMMZ(1)(sg,e,tp,mg) and Duel.IsExistingTarget(s.spfilter,tp,LOCATION_REST,0,1,sg,e,tp)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local g=Duel.GetMatchingGroup(s.cfilter,tp,LOCATION_REST|LOCATION_MZONE,0,nil)
