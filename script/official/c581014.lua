...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		if not (chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp)) then return false end
		local op=e:GetLabel()
		if op==1 then
	...
	local b1=Duel.IsPlayerCanDraw(tp,1)
		and Duel.IsExistingTarget(aux.AND(Card.IsMonster,Card.IsAbleToDeck),tp,LOCATION_REST,0,3,nil)
	...
	if op==1 then
		...
		local g=Duel.SelectTarget(tp,aux.AND(Card.IsMonster,Card.IsAbleToDeck),tp,LOCATION_REST,0,3,3,nil)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	local op=e:GetLabel()
	if op==1 then
		--Shuffle 3 monsters from your RP into the Deck, then draw 1 card
	...
	elseif op==2 then
		--Special Summon 1 non-Effect Monster from your RP
