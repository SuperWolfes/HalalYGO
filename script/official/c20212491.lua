...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil,e,tp)
	if #g>0 then
...
function s.attachop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsType(TYPE_XYZ) then
		...
		local g=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.atchfilter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil)
