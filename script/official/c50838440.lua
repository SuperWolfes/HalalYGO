...
function s.initial_effect(c)
	--Special Summon 3 Normal Monsters from your RP
	...
	c:RegisterEffect(e1)
	--Set 1 appropriate Actional/Trap from your hand or Deck
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>2
		...
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST,0,3,nil,e,tp) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_REST,0,3,3,nil,e,tp)
	if #g>0 then
...
function s.setfilter(c,codes)
	return c:IsActionalTrap() and c:IsSSetable() and c:ListsCode(codes[1]) and c:ListsCode(codes[2]) and c:ListsCode(codes[3])
end
