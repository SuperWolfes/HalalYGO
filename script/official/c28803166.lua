...
function s.initial_effect(c)
	--Send 1 "Taintedsmith" card from your Deck to the RP
	...
	c:RegisterEffect(e2)
	--Special Summon 1 "Taintedsmith" Link Monster from your RP
...
function s.tgfilter(c)
	return c:IsSetCard(SET_TAINTEDSMITH) and not c:IsCode(id) and c:IsAbleToGrave()
end
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_TAINTEDSMITH) and c:IsLinkMonster() and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp) and s.spfilter(chkc,e,tp) and chkc~=c end
