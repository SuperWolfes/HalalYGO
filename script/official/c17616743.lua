...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local loc=LOCATION_HAND+LOCATION_REST
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if s.selchk(tp) and not Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_REST,0,1,nil,e,tp) then
		loc=LOCATION_DECK
	else
		if s.selchk(tp) and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp)
			...
			loc=LOCATION_HAND+LOCATION_REST
		end
	end
