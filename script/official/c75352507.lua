...
function s.initial_effect(c)
	...
	--Destroy up to 2 Actionals/Traps your opponent controls
...
function s.rescon(sg,e,tp,mg)
	return Duel.IsExistingTarget(Card.IsActionalTrap,tp,0,LOCATION_ONFIELD,#sg,nil)
end
function s.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	local rg=Duel.GetMatchingGroup(aux.AND(Card.IsMonster,Card.IsAbleToRemoveAsCost),tp,LOCATION_REST,0,nil)
	if chk==0 then return #rg>0 end
	local g=aux.SelectUnselectGroup(rg,e,tp,1,2,s.rescon,1,tp,HINTMSG_REMOVE)
	e:SetLabel(#g)
...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local g=Duel.SelectTarget(tp,Card.IsActionalTrap,tp,0,LOCATION_ONFIELD,ct,ct,nil)
