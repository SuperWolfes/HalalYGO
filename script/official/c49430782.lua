...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Draw cards equal to the number of "True Draco" and "True King" card types sent to the RP
	...
	c:RegisterEffect(e4)
	--Count the number of "True Draco" and "True King" card types sent to the RP each turn
...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	for tc in eg:Iter() do
		if tc:IsPreviousLocation(LOCATION_ONFIELD) and tc:IsSetCard(SET_TRUE_DRACO_KING) then
			local typ=(tc:GetOriginalType()&(TYPE_MONSTER|TYPE_ACTIONAL|TYPE_TRAP))
