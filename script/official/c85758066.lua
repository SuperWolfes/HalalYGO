...
s.listed_names={CARD_MONSTER_REREST,10000000}
function s.setcon(e,tp,eg,ep,ev,re,r,rp)
	local at=Duel.GetAttacker()
	return at:IsControler(1-tp)
end
function s.setfilter(c,ft)
	return c:IsActional() and c:IsSSetable(true) and (c:IsType(TYPE_FIELD) or ft>0)
end
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ft=Duel.GetLocationCount(tp,LOCATION_SZONE)
	if e:GetHandler():IsLocation(LOCATION_HAND) then ft=ft-1 end
	if chkc then return chkc:IsControler(1-tp) and chkc:IsLocation(LOCATION_REST) and s.setfilter(chkc,ft) end
	if chk==0 then return Duel.IsExistingTarget(s.setfilter,tp,0,LOCATION_REST,1,nil,ft) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
	local g=Duel.SelectTarget(tp,s.setfilter,tp,0,LOCATION_REST,1,1,nil,ft)
...
function s.spcfilter(c,tp)
	return c:IsCode(CARD_MONSTER_REREST) and c:IsAbleToRestAsCost() and (c:IsLocation(LOCATION_HAND) or c:IsFacedown())
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
