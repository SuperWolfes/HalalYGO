...
function s.thfilter(c,tp)
	return c:IsCanAddCounter(COUNTER_ACTIONAL,1,false,LOCATION_MZONE) and c:IsLevelAbove(1)
		and c:IsAbleToHand() and (c:IsFaceup() or c:IsLocation(LOCATION_DECK))
		and Duel.IsCanRemoveCounter(tp,1,0,COUNTER_ACTIONAL,c:GetLevel(),REASON_COST)
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.RemoveCounter(tp,1,0,COUNTER_ACTIONAL,lv,REASON_COST)
	e:SetLabel(lv)
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK+LOCATION_EXTRA)
end
function s.filter(c,lv)
	return c:IsCanAddCounter(COUNTER_ACTIONAL,1,false,LOCATION_MZONE) and c:IsLevel(lv)
		and c:IsAbleToHand() and (c:IsFaceup() or c:IsLocation(LOCATION_DECK))
end
