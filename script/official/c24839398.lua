...
function s.initial_effect(c)
	...
	--The equipped monster gains 400 ATK for each Trap in your RP
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_EQUIP)
	e1:SetCode(EFFECT_UPDATE_ATTACK)
	e1:SetValue(function(e) return Duel.GetMatchingGroupCount(Card.IsTrap,e:GetHandlerPlayer(),LOCATION_REST,0,nil)*400 end)
	c:RegisterEffect(e1)
...
function s.settg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.setfilter,tp,LOCATION_REST,0,1,nil,true) end
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_REST,nil,1,tp,0)
end
function s.vetop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
	local g=Duel.SelectMatchingCard(tp,s.setfilter,tp,LOCATION_REST,0,1,1,nil,false)
	if #g>0 then
		Duel.SSet(tp,g)
	end
end
