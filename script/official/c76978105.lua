...
function s.initial_effect(c)
	--Add 1 "Dark Fusion" from your Deck or RP to your hand
...
function s.setcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return c:IsAbleToRemoveAsCost()
		and Duel.IsExistingMatchingCard(aux.AND(Card.IsMonster,Card.IsAbleToRemoveAsCost),tp,LOCATION_REST,0,4,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,aux.AND(Card.IsMonster,Card.IsAbleToRemoveAsCost),tp,LOCATION_REST,0,4,4,c)
	g:AddCard(c)
