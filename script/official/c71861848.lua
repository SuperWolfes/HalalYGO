...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then
		if not (chkc:IsControler(tp) and chkc:IsLocation(LOCATION_REST)) then return false end
		if e:GetLabel()==0 then return s.spfilter(chkc,e,tp) else return s.thfilter(chkc) end
	end
	local b1=Duel.IsExistingTarget(s.spfilter,tp,LOCATION_REST,0,1,nil,e,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
