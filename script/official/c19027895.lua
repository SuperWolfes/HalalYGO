...
function s.initial_effect(c)
	--Special Summon 1 FIRE monster from the RP
...
function s.atkfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(SET_SALAMANGREAT) and Duel.IsExistingTarget(s.tdfilter,tp,LOCATION_REST,0,1,nil,c:GetCode())
end
...
function s.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local tdc=Duel.SelectTarget(tp,s.tdfilter,tp,LOCATION_REST,0,1,1,nil,tc:GetFirst():GetCode())
	Duel.SetOperationInfo(0,CATEGORY_TODECK,tdc,1,tp,0)
end
function s.tdop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetTargetCards(e)
	if #g==0 then return end
	local tc=g:Filter(Card.IsLocation,nil,LOCATION_REST):GetFirst()
