--魔鍵憑霊－ウェパルトゥ
--Menkey Guardian - Vepartu
--Scripted by Eerie Code
local s,id=GetID()
function s.initial_effect(c)
	c:EnableAwakeLimit()
	Xyz.AddProcedure(c,nil,4,2)
	--Add 1 Normal monster from the Deck or RP to the hand
	...
	--Make the opponent send a monste to the RP
...
function s.gyfilter(c,att)
	return c:IsMonster() and (c:IsType(TYPE_NORMAL) or c:IsSetCard(SET_MENKEY)) and c:IsAttribute(att)
end
function s.gycon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:GetOverlayGroup():IsExists(Card.IsType,1,nil,TYPE_NORMAL)
		...
		and Duel.IsExistingMatchingCard(s.gyfilter,tp,LOCATION_REST,0,1,nil,bc:GetAttribute())
end
function s.gytg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOREST,e:GetHandler():GetBattleTarget(),1,0,0)
end
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	local bc=e:GetHandler():GetBattleTarget()
	if bc:IsRelateToBattle() and bc:IsFaceup() then
		Duel.SendtoRest(bc,REASON_RULE,PLAYER_NONE,1-tp)
	end
end