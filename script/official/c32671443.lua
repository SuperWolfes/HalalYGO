...
function s.costfilter(c,e,tp,ft)
	return c:IsSetCard(0xc008) and c:IsMonster() and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
		and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_REST,0,1,c,e,tp,ft)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.filter),tp,LOCATION_REST,0,1,1,nil,e,tp,ft):GetFirst()
	if tc then
