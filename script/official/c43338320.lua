...
function s.initial_effect(c)
	...
	c:RegisterEffect(e0)
	-- Prevent Actional/Trap activations during battle
	...
	c:RegisterEffect(e1)
	-- Special Summon 1 "Memento" monster from your hand or RP 
	...
	c:RegisterEffect(e2)
	-- Set 1 "Memento" Actional/Trap from your RP to your field
...
function s.tcfilter(c,e,tp)
	return c:IsPreviousLocation(LOCATION_MZONE) and c:IsPreviousControler(tp) and c:IsReason(REASON_BATTLE|REASON_EFFECT)
		...
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND|LOCATION_REST,0,1,nil,e,tp,c:GetLevel())
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_REST,0,1,1,nil,e,tp,tc:GetLevel())
	if #g>0 then
		Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	end
end
function s.setfilter(c)
	return c:IsSetCard(SET_MEMENTO) and c:IsActionalTrap() and c:IsSSetable()
end
