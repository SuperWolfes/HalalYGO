...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		if not Duel.IsExistingTarget(s.ecfilter,tp,LOCATION_REST,0,1,nil) then return false end
	...
	local g=Duel.SelectTarget(tp,s.ecfilter,tp,LOCATION_REST,0,1,1,nil)
	local ec=g:GetFirst()
	e:SetLabelObject(ec)
