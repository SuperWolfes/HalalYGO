...
function s.gytg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(1-tp) and chkc:IsOnField() and chkc:IsAbleToRest() end
	if chk==0 then return Duel.IsExistingTarget(Card.IsAbleToRest,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOREST)
	local g=Duel.SelectTarget(tp,Card.IsAbleToRest,tp,0,LOCATION_ONFIELD,1,1,nil)
...
function s.mltg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=Duel.GetMatchingGroup(Card.IsRace,tp,LOCATION_REST,0,nil,RACE_REPTILE):GetClassCount(Card.GetCode)
