...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct>0 and Duel.GetMZoneCount(tp,mg)>=ct and mg:FilterCount(Card.IsAbleToRest,nil)==ct and #sg>=ct
		and (ct==1 or not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN)) end
	Duel.SetOperationInfo(0,CATEGORY_TOREST,mg,ct,tp,0)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct=#og
	if ct==0 or (ct>1 and Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN)) then return end
