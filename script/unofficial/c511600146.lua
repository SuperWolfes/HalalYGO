...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_DRAW+CATEGORY_TOREST)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Draw(tp,1,REASON_EFFECT)>0 then
		...
		if tc:IsMonster() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST,0,1,nil,e,tp,tc:GetLevel())
