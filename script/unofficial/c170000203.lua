...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Make this card's effect the effect of banished Trap
...
function s.con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_HAND+LOCATION_REST,0,1,nil)
	and e:GetLabel()~=1 and e:GetHandler()==Duel.GetAttacker() and e:GetHandler():GetBattleTarget()~=nil
	or Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_HAND+LOCATION_REST,0,1,nil)
...
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_HAND+LOCATION_REST,0,1,nil) end
	local tc=Duel.SelectMatchingCard(tp,s.filter1,tp,LOCATION_HAND+LOCATION_REST,0,1,1,nil)
	e:SetLabel(1)
