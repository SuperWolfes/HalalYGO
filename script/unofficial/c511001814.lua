...
s.fit_monster={70345785}
function s.filter(c,e,tp,m)
	local cd=c:GetCode()
	if cd~=70345785 or not c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LOCKED,tp,true,false) then return false end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(s.filter,tp,0x33,0,1,nil,e,tp,mg)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.filter),tp,0x33,0,1,1,nil,e,tp,mg)
	if #tg>0 then
