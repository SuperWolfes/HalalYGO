...
function s.filter(c,tp)
	...
	return c:GetLevel()>4 and c:IsClear()
		and Duel.IsExistingMatchingCard(s.rfilter,tp,LOCATION_MZONE+LOCATION_REST,0,mi,nil) and not c:IsPublic()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local rg=Duel.SelectMatchingCard(tp,s.rfilter,tp,LOCATION_MZONE+LOCATION_REST,0,mi,ma,nil)
