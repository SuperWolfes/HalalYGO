...
function s.filter1(c,e,tp)
	...
	return (c:IsLocation(LOCATION_MZONE) and c:IsFaceup() and c:IsCanBeEffectTarget(e))
		or (c:IsLocation(LOCATION_REST|LOCATION_EXTRA) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE|LOCATION_REST|LOCATION_EXTRA) and s.filter1(chkc,e,tp) end
	if chk == 0 then return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_MZONE|LOCATION_REST|LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local tc = Duel.SelectMatchingCard(tp,s.filter1,tp,LOCATION_MZONE|LOCATION_REST|LOCATION_EXTRA,0,1,1,nil,e,tp):GetFirst()
