...
function s.filter(c,tid)
	return c:IsTrap() and c:IsAbleToHand() and c:GetTurnID()==tid and c:IsPreviousLocation(LOCATION_ONFIELD)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local tid=Duel.GetTurnCount()
	if chkc then return chkc:GetLocation()==LOCATION_REST and chkc:GetControler()==tp and s.filter(chkc,tid) end
	if chk==0 then return Duel.IsExistingTarget(s.filter,tp,LOCATION_REST,0,1,nil,tid) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,s.filter,tp,LOCATION_REST,0,1,1,nil,tid)
