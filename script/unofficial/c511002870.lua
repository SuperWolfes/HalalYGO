...
--Number 102: Star Watcher Sentry (Anime)
...
function s.initial_effect(c)
	...
	--Detach all materials from this to prevent this card's mismatching (Battle)
	...
	c:RegisterEffect(e3)
	--Detach all materials from this to prevent this card's mismatching (Card effect)
	local e4=e3:Clone()
...
function s.indesop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		...
		if e:GetCode()==EVENT_PRE_DAMAGE_CALCULATE then
			...
			e2SetProperty(EFFECT_FLAG_PLAYER_TARGET)
