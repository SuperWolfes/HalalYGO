...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		elseif suffice==2 and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsActional),tp,LOCATION_ONFIELD,0,1,e:GetHandler()) then
		...
		elseif suffice==3 and Duel.IsExistingMatchingCard(Card.IsMonster,tp,LOCATION_MZONE,0,1,e:GetHandler()) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
			local dg=Duel.SelectMatchingCard(tp,Card.IsMonster,tp,LOCATION_MZONE,0,1,1,nil)
			Duel.Destroy(dg,REASON_EFFECT)
		--Destroy 1 monster your opponent controls
		elseif suffice==4 and Duel.IsExistingMatchingCard(Card.IsMonster,tp,0,LOCATION_MZONE,1,nil) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
			local dg=Duel.SelectMatchingCard(tp,Card.IsMonster,tp,0,LOCATION_MZONE,1,1,nil)
			Duel.Destroy(dg,REASON_EFFECT)
		--Destroy 1 Actional your opponent controls
		elseif suffice==5 and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsActional),tp,0,LOCATION_ONFIELD,1,e:GetHandler()) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
