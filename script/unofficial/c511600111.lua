...
function s.filter(c,e,tp)
	return c:IsFaceup() and c:IsLinkMonster()
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST,0,1,c,e,tp,c:GetAttack())
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.Destroy(tc,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(aux.GraveValleyFilter(s.spfilter),tp,LOCATION_REST,0,1,nil,e,tp,tc:GetPreviousAttackOnField()) then
		Duel.BreakEffect()
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sc=Duel.SelectMatchingCard(tp,aux.GraveValleyFilter(s.spfilter),tp,LOCATION_REST,0,1,1,nil,e,tp,tc:GetPreviousAttackOnField()):GetFirst()
