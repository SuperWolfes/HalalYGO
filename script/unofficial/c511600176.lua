...
function s.tktg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.filter,tp,LOCATION_MZONE,0,1,nil,e,tp)
		and Duel.IsPlayerCanSpecialSummonMonster(tp,51208047,0xfb,TYPES_TOKEN,0,0,1,RACE_WANDERER,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENSE) end
...
function s.tkop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,51208047,0xfb,TYPES_TOKEN,0,0,1,RACE_WANDERER,ATTRIBUTE_LIGHT,POS_FACEUP_DEFENSE) then return end
	local token=Duel.CreateToken(tp,51208047)
