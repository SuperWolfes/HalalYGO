...
function s.filter(c,e,tp)
	...
	return c:IsSetCard(0x48) and c:IsType(TYPE_XYZ)
		...
		and Duel.IsExistingMatchingCard(s.atfilter,tp,LOCATION_REST,0,ct,c)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		local og=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.atfilter),tp,LOCATION_REST,0,ct,ct2,nil)
		Duel.Overlay(tc,og)
	end
end
