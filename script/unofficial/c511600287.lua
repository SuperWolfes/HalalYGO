...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_DRAW+CATEGORY_TOREST+CATEGORY_REMOVE)
...
function s.filter(c)
	return c:IsSetCard(0x70) and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE+LOCATION_REST) and chkc:IsControler(tp) and s.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.filter,tp,LOCATION_MZONE+LOCATION_REST,0,1,nil) and Duel.IsPlayerCanDraw(tp,2) end
