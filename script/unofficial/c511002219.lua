...
function s.filter(c,tp,mc)
	return c:IsType(TYPE_SYNCHRO) and Duel.IsExistingMatchingCard(s.matfilter,tp,LOCATION_REST,0,1,nil,mc,c)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		local mat=Duel.SelectMatchingCard(tp,s.matfilter,tp,LOCATION_REST,0,1,1,nil,c,tc)
		mat:AddCard(c)
		Synchro.Send=2
