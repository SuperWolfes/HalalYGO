--究極封印神エクゾディオス (Anime)
--Exdudius the Ultimate Forbidden Watcher (Anime)
local s,id=GetID()
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Must be Special Summoned with "Ultimate Locked of the Forbidden Watcher"
	...
	c:RegisterEffect(e3)
	--Send 1 "Forbidden One" monster from your hand or Deck to the RP when this card attacks
	...
	c:RegisterEffect(e4)
	--Gains 1000 ATK for each "Forbidden One" card in your RP
	local e5=e2:Clone()
	e5:SetCode(EFFECT_UPDATE_ATTACK)
	e5:SetValue(s.atkval)
	c:RegisterEffect(e5)
	--Win the Duel when there are 5 "Forbidden One" cards with different names in your RP
...
function s.tgfilter(c)
	return c:IsSetCard(SET_UNLIKED_ONE) and c:IsMonster() and c:IsAbleToGrave()
end
