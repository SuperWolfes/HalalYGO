...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ct1+ct2>0 and Duel.GetFieldGroupCount(tp,LOCATION_REST,0)>=ct1 
		and Duel.GetFieldGroupCount(tp,0,LOCATION_REST)>=ct2 end
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local ct1=Duel.GetFieldGroupCount(tp,LOCATION_REST,0)
	local ct2=Duel.GetFieldGroupCount(tp,0,LOCATION_REST)
	if ct1<#g1 
		or ct2<#g2 then return end
	if #g1>0 then
		local og=Group.CreateGroup()
		for i=1,#g1 do
			local tc=Duel.GetFieldCard(tp,LOCATION_REST,ct1-i)
			og:AddCard(tc)
		end
	...
	if #g2>0 then
		local og=Group.CreateGroup()
		for i=1,#g2 do
			local tc=Duel.GetFieldCard(1-tp,LOCATION_REST,ct2-i)
			og:AddCard(tc)
		end
