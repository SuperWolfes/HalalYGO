...
function card.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON + CATEGORY_LEAVE_REST + CATEGORY_TOHAND + CATEGORY_SEARCH)
...
function card.sptg(e, tp, eg, ep, ev, re, r, rp, chk)
	...
	if chk == 0 then
		return aux.SelectUnselectGroup(g, e, tp, 2, 2, card.rescon, chk) and
			not Duel.IsPlayerAffectedByEffect(tp, CARD_BLUEEYES_GUARDIAN) and
...
function card.spop(e, tp, eg, ep, ev, re, r, rp)
	if Duel.GetLocationCount(tp, LOCATION_MZONE) < 2 or Duel.IsPlayerAffectedByEffect(tp, CARD_BLUEEYES_GUARDIAN) then
		return
	end
	local g = Duel.GetMatchingGroup(card.spfilter, tp, LOCATION_REST, 0, nil, e, tp)
