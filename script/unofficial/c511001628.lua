...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		e:SetLabel(0)
		return Duel.IsExistingMatchingCard(s.filter,tp,0x13,LOCATION_REST,1,nil,e,tp)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.filter),tp,0x13,LOCATION_REST,1,1,nil,e,tp)
	if #g>0 then
