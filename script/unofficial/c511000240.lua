...
function s.initial_effect(c)
	c:EnableReviveLimit()
	--"Exdude the Forbidden One": This card cannot be destroyed by battle
	...
	e2:SetValue(1)
	c:RegisterEffect(e2)
	--"Left Leg of the Forbidden One": This card cannot be destroyed by Actional Cards.
	...
	c:RegisterEffect(e6)
	--Revert ATK changes if removed from field/if no Right Arm in RP
...
function s.atkcon(e)
	return Duel.IsExistingMatchingCard(Card.IsCode,e:GetHandlerPlayer(),LOCATION_REST,0,1,nil,70903634)
end
...
function s.resetatk(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REST,0,1,nil,70903634) then return end
	local c=e:GetHandler()
