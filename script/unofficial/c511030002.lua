...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and tc:IsFacedown() then
		...
		if Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_DECK+LOCATION_REST,0,1,nil,code)
			and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.BreakEffect()
			local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thfilter),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil,code)
			if #g>0 then
...
function s.sttg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp) and s.stfilter(chkc,tid) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(s.stfilter,tp,LOCATION_REST,0,1,nil,tid) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SET)
	local g=Duel.SelectTarget(tp,s.stfilter,tp,LOCATION_REST,0,1,1,nil,tid)
