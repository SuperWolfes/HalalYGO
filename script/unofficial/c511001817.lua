...
s.fit_monster={76232340}
function s.filter(c,e,tp,m)
	local cd=c:GetCode()
	if cd~=76232340 or not c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_LOCKED,tp,false,true) then return false end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.filter),tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,e,tp,mg)
	if #tg>0 then
