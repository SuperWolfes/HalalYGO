...
function s.cfilter(c)
	return c:IsRace(RACE_MENTAL) and c:IsType(TYPE_PENDULUM) and c:IsAbleToRemoveAsCost()
end
...
function s.nbfilter(c)
	return not c:IsRace(RACE_MENTAL)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	elseif pentg:FilterCount(s.nbfilter,nil)>0 or pct>=4 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
		g=sg:Select(tp,3,3,nil)
	elseif pct==2 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
