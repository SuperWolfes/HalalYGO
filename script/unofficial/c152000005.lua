...
function s.rmvfilter1(c,e,tp)
	return c:IsLevelBelow(2) and c:IsAbleToRemove() and aux.SpElimFilter(c,true)
		and Duel.IsExistingMatchingCard(s.rmvfilter2,tp,LOCATION_REST+LOCATION_MZONE,0,2,nil,e,tp,c)
end
...
function s.flipcon(e,tp,eg,ep,ev,re,r,rp)
	--condition
	return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) and Duel.IsExistingMatchingCard(s.rmvfilter1,tp,LOCATION_REST+LOCATION_MZONE,0,2,nil,e,tp)
end
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Special Summon
	local g1=Duel.GetMatchingGroup(s.rmvfilter1,tp,LOCATION_REST+LOCATION_MZONE,0,nil,e,tp)
	if #g1<=0 then return end
	local rg=aux.SelectUnselectGroup(g1,e,tp,2,2,s.rescon,1,tp,HINTMSG_REMOVE)
	local lv=rg:GetFirst():GetLevel()
	if Duel.Remove(rg,POS_FACEUP,REASON_EFFECT)>0 and not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) then
