...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,0,-2,0,1,RACE_CONTAMINED,ATTRIBUTE_DARK) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,0,-1,0,1,RACE_CONTAMINED,ATTRIBUTE_DARK) then return end
	Duel.BreakEffect()
