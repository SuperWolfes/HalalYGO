...
function s.filter1(c,e,tp)
	...
	return c:IsLevelBelow(8) and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,nil,lv1,tp)
end
function s.filter2(c,lv1,tp)
	...
	return c:IsAbleToRemove() and c:IsType(TYPE_TUNER) and aux.SpElimFilter(c,true)
		and Duel.IsExistingMatchingCard(s.filter3,tp,LOCATION_MZONE+LOCATION_REST,0,1,c,lv1-lv2)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g2=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,nil,tg1:GetLevel(),tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g3=Duel.SelectMatchingCard(tp,s.filter3,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,nil,tg1:GetLevel()-g2:GetFirst():GetLevel())
	g2:Merge(g3)
