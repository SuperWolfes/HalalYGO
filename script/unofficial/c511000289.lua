...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_REST,0,1,1,nil,e,tp)
	Duel.SpecialSummonStep(g1:GetFirst(),0,tp,tp,false,false,POS_FACEUP)
end
	if Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0
	and Duel.IsExistingMatchingCard(s.filter,1-tp,LOCATION_REST,0,1,nil,e,1-tp)
	and Duel.SelectYesNo(1-tp,aux.Stringid(id,1)) then
	Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_SPSUMMON)
	local g2=Duel.SelectMatchingCard(1-tp,s.filter,1-tp,LOCATION_REST,0,1,1,nil,e,1-tp)
