...
function s.stage2(e,tc,tp,sg,chk)
	if chk==1 and e:IsHasType(EFFECT_TYPE_ACTIVATE) then
		...
		e1:SetReset(RESET_EVENT+RESETS_STANDARD_EXC_REST+RESET_PHASE+PHASE_END+RESET_SELF_TURN,res)
		c:RegisterEffect(e1)
		...
		e2:SetReset(RESET_EVENT+RESETS_STANDARD_EXC_REST+RESET_PHASE+PHASE_END+RESET_SELF_TURN,res)
		c:RegisterEffect(e2)
	end
end
