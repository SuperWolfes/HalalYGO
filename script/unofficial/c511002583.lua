...
function s.initial_effect(c)
	--Special Summon 2 Rock monsters from the RP
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		if chkcost then
			e:SetLabel(0)
			return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE+LOCATION_REST,0,1,nil,e,tp)
		else
			return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_REST,0,2,c,e,tp)
		end
	end
	if chkcost then
		local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,nil,e,tp)
