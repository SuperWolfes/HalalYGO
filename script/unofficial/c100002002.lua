...
function s.matfilter(c,e,tp)
	if not c:IsRace(RACE_TOXIC) then return false end
	local loc=c:GetLocation()
	return (loc&(LOCATION_DECK+LOCATION_MZONE)~=0 and c:IsAbleToGrave()) or loc&LOCATION_REST~=0
end
...
function s.extraop(e,tc,tp,sg)
	Duel.SendtoGrave(sg:Filter(aux.NOT(Card.IsLocation),nil,LOCATION_REST),REASON_EFFECT+REASON_MATERIAL+REASON_FUSION)
	sg:Clear()
end
