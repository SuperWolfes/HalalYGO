...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND|LOCATION_REST|LOCATION_DECK,0,1,nil,e,tp) end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_REST|LOCATION_DECK,0,1,1,nil,e,tp):GetFirst()
	if tc then
...
function s.respop(e,tp,eg,ep,ev,re,r,rp)
	...
	for tc in g:Iter() do
		...
		e1:SetRange(LOCATION_REST+LOCATION_REMOVED)
