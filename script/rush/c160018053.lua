...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Can also shuffle monsters from your RP as Fusion Material for the Fusion Summon using "Fusion"
...
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsAbleToRestAsCost() and c:IsRace(RACE_WYRM) and c:IsLevelAbove(7) 
end
...
function s.extrafil_replacement(e,tp,mg)
    return Duel.GetMatchingGroup(aux.RestValleyFilter(s.extrafil_repl_filter),tp,LOCATION_REST,0,nil)
end
