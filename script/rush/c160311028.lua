...
function s.initial_effect(c)
	--Special Summon 1 Galaxy Normal Monster from the RP
...
function s.filter(c,e,tp)
	return c:IsRace(RACE_GALAXY) and c:IsType(TYPE_NORMAL) and c:IsAttackAbove(1600) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_ATTACK)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.filter),tp,LOCATION_REST+LOCATION_HAND,0,1,1,nil,e,tp)
	if #g>0 then
