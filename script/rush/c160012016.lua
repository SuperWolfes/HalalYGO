...
function s.filter(c,e,sp)
	return c:IsRace(RACE_BEAST) and c:IsDefense(200) and c:IsCanBeSpecialSummoned(e,0,sp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.filter),tp,LOCATION_REST,0,1,nil,e,tp) then
		...
		if #g>0 and Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)>0 and g:GetFirst():IsType(TYPE_NORMAL) then
			local g2=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.thfilter),tp,LOCATION_REST,0,nil)
