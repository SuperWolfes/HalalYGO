...
function s.costfilter(c)
	return c:IsMonster() and c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsAbleToRestAsCost()
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Draw(p,d,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(Card.IsType,tp,LOCATION_REST,0,1,nil,TYPE_NORMAL) 
