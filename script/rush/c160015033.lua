--絶望狂魔デッド・エンド
--Dead End the Mad Tainted of Despair
...
function s.attfilter(c,tp)
	return Duel.IsExistingMatchingCard(Card.IsAttribute,tp,LOCATION_REST,0,5,c,c:GetAttribute())
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if ct>0 then
			...
			if #g2>0 and Duel.IsExistingMatchingCard(s.attfilter,tp,LOCATION_REST,0,1,nil,tp) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
