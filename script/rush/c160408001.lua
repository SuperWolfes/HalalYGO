...
function s.thfilter(c)
	return c:IsRace(RACE_MENTOR) and c:IsAttribute(ATTRIBUTE_DARK) and c:IsLevelAbove(7) and c:IsAbleToHand()
end
...
function s.thfilter2(c)
	return c:IsRace(RACE_MENTOR) and c:IsLevelAbove(6) and c:IsAbleToHand()
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(tg,REASON_COST)==1 then
		...
		if #g>0 then
			...
			if g:GetFirst():IsCode(CARD_SEVENS_ROAD_MENTOR) and #g2>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
