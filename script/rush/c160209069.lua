...
function s.thfilter(c)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:IsRace(RACE_MENTAL) and c:IsDefense(0) and c:IsAbleToHand()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #dg>0 then
		...
		if dg:GetFirst():IsCode(160209059) and Duel.IsPlayerCanDiscardDeck(tp,3) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.BreakEffect()
