...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_REST,2,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TODECK,nil,2,tp,LOCATION_REST)
end
function s.spfilter(c,e,tp)
	return (c:IsCode(CARD_BIA_CHAMPION) or (c:IsRace(RACE_AQUA) and c:IsDefense(2000))) 
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.filter),tp,0,LOCATION_REST,2,2,nil)
