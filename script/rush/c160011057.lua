--掃神降臨
--Descent of the Sweeping Guardian
...
function s.revfilter(c)
	return c:IsMonster() and c:IsRace(RACE_WANDERER) and not c:IsPublic()
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.revfilter,tp,LOCATION_HAND,0,2,nil) end
end
function s.filter(c,e,tp)
	return c:IsRace(RACE_WANDERER) and c:IsLevelAbove(7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
