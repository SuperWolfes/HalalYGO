...
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_TOXIC) and c:IsLevel(1) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
		and Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,3,c)
end
function s.tdfilter(c)
	return c:IsRace(RACE_TOXIC) and not c:IsLevel(4) and c:IsAbleToDeckOrExtraAsCost()
end
