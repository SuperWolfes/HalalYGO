...
function s.cfilter(c)
	return c:IsFaceup() and c:IsRace(RACE_TOXIC) and c:IsNotMaximumModeSide() and c:IsAbleToGraveAsCost()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #dg>0 then
		...
		if Duel.Destroy(sg,REASON_EFFECT)>0 and Duel.GetMatchingGroupCount(Card.IsRace,tp,LOCATION_REST,0,nil,RACE_TOXIC)>=4
