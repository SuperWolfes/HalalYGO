...
function s.initial_effect(c)
	--Return Sea Serpent monsters from the RP to the Deck
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_REST,0,1,3,nil)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and #sg>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		...
		if #ssg>0 then
			...
			if Duel.SpecialSummon(ssg,0,tp,tp,false,false,POS_FACEUP_ATTACK)>0
				and Duel.GetMatchingGroupCount(Card.IsMonster,tp,LOCATION_REST,0,nil)==0
