...
function s.mfilter(c)
	return c:IsLocation(LOCATION_REST) and c:IsAbleToDeck()
end
...
function s.fextra(e,tp,mg)
	return Duel.GetMatchingGroup(s.mfilter,tp,LOCATION_REST,0,nil),s.fcheck
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE,1-tp)>0
		and Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_REST,1,nil,e,1-tp) end
...
function s.operation(fustg,fusop)
	return function(e,tp,eg,ep,ev,re,r,rp)
		...
		local sg=Duel.SelectMatchingCard(1-tp,s.filter,tp,0,LOCATION_REST,1,1,nil,e,1-tp)
