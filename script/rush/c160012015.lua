...
function s.costfilter(c)
	return c:IsMonster() and c:IsRace(RACE_BEAST|RACE_WARRIOR) and c:IsDefense(200) and c:IsAbleToRestAsCost()
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if dg:GetFirst():IsType(TYPE_NORMAL) then
		c:AddDoubleTribute(id,s.otfilter,s.eftg,RESETS_STANDARD_PHASE_END,FLAG_DOUBLE_TRIB_200_DEF)
	end
end
