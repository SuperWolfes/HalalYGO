...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdcfilter,tp,LOCATION_REST,0,3,nil) end
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	-- shuffle 3 monsters from your Resting Place into the Deck. to shuffle 3 monsters from opponent's GY to deck
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	--Requirement
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,s.tdcfilter,tp,LOCATION_REST,0,3,3,nil)
	if #g==0 then return end
	Duel.HintSelection(g,true)
	if Duel.SendtoDeck(g,nil,SEQ_DECKSHUFFLE,REASON_COST)>0 then
		--Effect
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
		local dg1=Duel.SelectMatchingCard(tp,s.tdfilter,tp,0,LOCATION_REST,3,3,nil)
		...
		if Duel.SendtoDeck(dg1,nil,SEQ_DECKSHUFFLE,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_MZONE,0,2,nil)
			and Duel.IsExistingMatchingCard(s.tdfilter,tp,0,LOCATION_REST,3,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
			local dg2=Duel.SelectMatchingCard(tp,s.tdfilter,tp,0,LOCATION_REST,3,3,nil)
...
function s.cfilter(c)
	return c:IsFaceup() and c:IsRace(RACE_MENTOR) and not c:IsLevel(1)
end