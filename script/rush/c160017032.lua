...
function s.cost(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(Card.IsAbleToRestAsCost,tp,LOCATION_ONFIELD,0,1,nil)
end
function s.filter(c)
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	--Requirement
	local ct=Duel.GetMatchingGroupCount(s.filter,tp,0,LOCATION_REST,nil)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOREST)
	local tg=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_ONFIELD,0,1,math.min(2,ct),nil)
	local ct2=Duel.SendtoRest(tg,REASON_COST)
	if ct2<1 then return end
	--Effect
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.filter),tp,0,LOCATION_REST,ct2,ct2,nil)
