...
function s.costfilter(c)
	return (c:IsActional() or (c:IsMonster() and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MENTOR))) and c:IsAbleToGraveAsCost()
end
...
function s.cfilter(c)
	return c:IsLocation(LOCATION_REST) and c:IsRace(RACE_MENTOR) and c:IsAttribute(ATTRIBUTE_LIGHT)
end
