...
function s.drcostfilter(c)
	return c:IsMonster() and c:IsAttribute(ATTRIBUTE_WIND) and c:IsAbleToRestAsCost()
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(tc,REASON_COST)==1 and Duel.Draw(tp,1,REASON_EFFECT)>0
		and #g>1 and Duel.IsPlayerCanDraw(tp,1) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then 
		Duel.BreakEffect()
