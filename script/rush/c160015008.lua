...
function s.initial_effect(c)
	--Send the top 4 cards from your Deck to the RP and add 1 card from the RP to the hand
...
s.listed_names={CARD_CELEB_ROSE_MENTOR,CARD_CELEB_ROSE_MINT,CARD_FUSION}
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsStatus(STATUS_SUMMON_TURN) 
end
function s.revfilter(c)
	return ((c:IsRace(RACE_MENTOR) and c:IsAttribute(ATTRIBUTE_LIGHT)
		and c:IsMonster()) or c:IsEquipActional()) and not c:IsPublic()
end
...
function s.cfilter(c)
	return c:IsLocation(LOCATION_REST) and c:IsCode(CARD_CELEB_ROSE_MENTOR,CARD_CELEB_ROSE_MINT,CARD_FUSION) and c:IsAbleToHand()
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:FilterCount(aux.RestValleyFilter(s.cfilter),nil)>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
