...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return not c:IsCode(CARD_DARK_MENTOR) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	c:RegisterEffect(e1)
	if Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,10,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
		local tg=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_REST,0,10,10,nil)
