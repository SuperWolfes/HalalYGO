--トランザム・ネビュラ・フュージョン
--Transamu Nebula Fusion
--Scripted by YoshiDuels
local s,id=GetID()
function s.initial_effect(c)
	--Send to Resting Place and Fusion Summon
...
function s.matfilter(c)
	return (c:IsLocation(LOCATION_MZONE) or (c:IsLocation(LOCATION_REST) and c:IsRace(RACE_GALAXY))) and c:IsAbleToDeck()
end
function s.checkmat(tp,sg,fc)
	return sg:FilterCount(Card.IsLocation,nil,LOCATION_MZONE)==1
end
function s.fextra(e,tp,mg)
	return Duel.GetMatchingGroup(s.matfilter,tp,LOCATION_REST|LOCATION_MZONE,0,nil),s.checkmat
end
function s.disfilter(c)
	return c:IsRace(RACE_GALAXY) and c:IsAbleToRest()
end
...
function s.operation(fustg,fusop)
	return function(e,tp,eg,ep,ev,re,r,rp)
		...
		if Duel.SendtoRest(tg,REASON_EFFECT)>0 and fustg(e,tp,eg,ep,ev,re,r,rp,0) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.BreakEffect()
