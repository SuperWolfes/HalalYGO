...
function s.tdfilter(c,tp)
	...
	return c:IsAbleToDeckOrExtraAsCost()
		and Duel.IsExistingMatchingCard(s.tdfilter2,tp,LOCATION_REST,0,1,c,race)
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,1,nil,tp)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_REST,0,1,1,nil,tp)
	local tc1=g1:GetFirst()
	local race=tc1:GetRace()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOREST)
	local g2=Duel.SelectMatchingCard(tp,s.tdfilter2,tp,LOCATION_REST,0,1,1,tc1,race)
	g1:Merge(g2)
