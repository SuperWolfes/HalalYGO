...
function s.initial_effect(c)
	--Draw 1 card and add 1 "Flybie Lady" or "Flybie Ladies" from the RP to the hand
...
s.listed_names={CARD_FLYBIE_LADY,160208002}
...
function s.thfilter(c)
	return c:IsCode(CARD_FLYBIE_LADY,160208002) and c:IsAbleToHand()
end
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoRest(g,REASON_COST)>0 then
		...
		if Duel.Draw(p,d,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_MZONE,0,1,nil)
			...
			local thg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thfilter),tp,LOCATION_REST,0,1,1,nil)
			if #thg>0 then
				Duel.BreakEffect()
