...
function s.initial_effect(c)
	--Special Summon 1 Level 7 Mental monster from your hand
...
function s.cfilter(c,e,tp)
	return c:IsAbleToRestAsCost() and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND,0,1,c,e,tp)
end
function s.spfilter(c,e,tp)
	return c:IsRace(RACE_MENTAL) and c:IsLevel(7) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #rg>0 and Duel.SpecialSummon(rg,0,tp,tp,false,false,POS_FACEUP)>0 and Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REST,0,1,nil,CARD_CAN_D)
