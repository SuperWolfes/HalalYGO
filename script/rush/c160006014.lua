...
function s.filter(c,atk)
	return c:IsFaceup() and c:GetAttack()==atk and c:GetBaseAttack()>0 and c:IsLevelBelow(8) and not c:IsMaximumModeSide()
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct>0 then
		...
		if #dg>0 then
			...
			local ct=Duel.GetMatchingGroupCount(Card.IsCode,tp,LOCATION_REST,0,nil,CARD_SEVENS_ROAD_MAGICIAN)
