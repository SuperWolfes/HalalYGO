...
function s.drcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REST,0,1,nil,31122090)
end
...
function s.drop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsPlayerCanDiscardDeck(tp,1) then
		...
		if ct then
			...
			if ct:IsMonster() and Duel.IsExistingMatchingCard(s.filter,tp,0,LOCATION_MZONE,2,nil,ct:GetRace()) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
