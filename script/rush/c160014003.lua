...
s.toss_coin=true
function s.filter(c)
	return c:IsMonster() and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsRace(RACE_MACHINE) and c:IsAbleToRestAsCost()
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(tc,REASON_COST)<1 then return end
	--Effect
