...
function s.initial_effect(c)
	...
	-- Add to hand or Special Summon 1 Level 7 EARTH Machine monster from the RP
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_REST,0,1,1,nil,e,tp):GetFirst()
	if tc then
		...
		if tc:IsCode(160014008) and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.thfilter),tp,LOCATION_REST,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
