...
function s.initial_effect(c)
	--Add 1 "Fusion" from the resting place to the hand
...
function s.thfilter2(c,tp)
	return c:IsType(TYPE_NORMAL) and c:IsMonster() and c:IsAbleToHand()
		and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REST,0,1,c,c:GetCode())
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #dg>0 then
		...
		local g=Duel.GetMatchingGroup(s.thfilter2,tp,LOCATION_REST,0,nil,tp)
		if #g>0 and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.BreakEffect()
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g2=Duel.SelectMatchingCard(tp,s.thfilter2,tp,LOCATION_REST,0,1,1,nil,tp)
			if #g2>0 then
