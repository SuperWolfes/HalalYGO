...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return not c:IsCode(CARD_SEVENS_ROAD_MENTOR) end
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	--Effect
	local c=e:GetHandler()
	--Name becomes "Sevens Road Mentor"
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e1:SetCode(EFFECT_CHANGE_CODE)
	e1:SetValue(CARD_SEVENS_ROAD_MENTOR)
	e1:SetReset(RESETS_STANDARD_PHASE_END)
	c:RegisterEffect(e1)
	local att=Duel.GetMatchingGroup(Card.IsMonster,tp,LOCATION_REST,0,nil):GetClassCount(Card.GetAttribute)
