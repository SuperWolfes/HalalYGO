...
function s.initial_effect(c)
	--Change position and Special Summon 1 "Flap Dragon" from the RP
...
function s.costfilter(c)
	return c:IsMonster() and c:IsAbleToRestAsCost() and c:IsRace(RACE_DRAGON)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(dg,REASON_COST)<1 then return end
	--Effect
