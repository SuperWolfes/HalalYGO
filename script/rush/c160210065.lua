...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoDeck(tg,nil,0,REASON_EFFECT)>0 then
		if Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REST,0,1,nil,160316023)
			and Duel.IsExistingMatchingCard(aux.NecroValleyFilter(s.tdfilter),tp,LOCATION_REST,0,2,nil) 
			and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
			local tg=Duel.SelectMatchingCard(tp,s.tdfilter,tp,LOCATION_REST,0,2,2,nil)
			Duel.HintSelection(tg,true)
