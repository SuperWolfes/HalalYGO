...
function s.initial_effect(c)
	--Name becomes "Etraynze the Shadow Flower Ninja" in the Resting Place
...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REST,0,1,nil,tp,e:GetHandler():GetCode()) end
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,s.cfilter,tp,LOCATION_REST,0,1,1,nil,tp,code)
	...
	if #g2>0 then
		...
		c:RegisterEffect(e1)
		local g3=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.setfilter),tp,LOCATION_REST,0,nil)
