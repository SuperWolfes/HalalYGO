...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.tgfilter,tp,LOCATION_ONFIELD,0,1,nil)
		and Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,2,nil) end
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(tg,REASON_COST)==0 or Duel.SendtoDeck(td,nil,SEQ_DECKSHUFFLE,REASON_COST)~=2 then return end
	Duel.ShuffleDeck(tp)
