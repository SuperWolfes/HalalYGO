--隕石竜の雛
--Meteorite Dragon's Chick
--scripted by YoshiDuels
local s,id=GetID()
function s.initial_effect(c)
	--Add to the hand 1 EARTH Dragon with 1800 ATK from the RP
...
function s.thfilter(c)
	return c:IsRace(RACE_DRAGON) and c:IsAttribute(ATTRIBUTE_EARTH) and c:IsAttack(1800) and c:IsAbleToHand()
end
