...
function s.costfilter(c)
	return c:IsFaceup() and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MENTOR)
		and c:IsAbleToRestAsCost() and not c:IsMaximumModeSide()
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_MZONE,0,3,nil) end
end
function s.filter(c,e,tp)
	return c:IsRace(RACE_MENTOR) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsAttackBelow(2500)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(g,REASON_COST)>0 then
		...
		if #g2>0 and Duel.SpecialSummon(g2,0,tp,tp,false,false,POS_FACEUP)>0 then
			local g3=Duel.GetMatchingGroup(aux.NecroValleyFilter(s.thfilter),tp,LOCATION_REST,0,nil)
