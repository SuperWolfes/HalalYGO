...
function s.cfilter(c)
	return c:IsRace(RACE_TOXIC) and c:IsFaceup() and not c:IsCode(53839837)
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Change Toxic monster to "Vampire Watcher"
...
function s.gytg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingTarget(s.gyfilter,tp,0,LOCATION_REST,1,nil,e,tp,ft) end
	local tc=Duel.SelectTarget(tp,s.gyfilter,tp,0,LOCATION_REST,1,1,nil,e,tp,ft):GetFirst()
...
function s.gyop(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif tc:IsActionalTrap() and tc:IsSSetable() then
			Duel.SSet(tp,tc,tp)
	end
end
