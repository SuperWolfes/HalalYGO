...
function s.costfilter(c,e,tp)
	return c:IsSetCard(SET_LV) and c:IsAbleToGraveAsCost() and c:IsFaceup() and c.listed_names and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_REST,0,1,nil,c,e,tp)
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoGrave(g,REASON_COST)>0 and g:GetFirst():IsLocation(LOCATION_REST) then
		...
		local sg=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_REST,0,1,1,nil,class,e,tp)
