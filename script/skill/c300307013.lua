...
function s.rfilter(c,tp)
	return c:IsReleasable() and c:IsRace(RACE_MENTAL|RACE_MACHINE)
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	--Tribute 1 Machine or Mental monster to Special Summon Jinzo
	...
	if #g>0 and Duel.Release(g,REASON_COST)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_HAND|LOCATION_REST,0,1,1,nil,e,tp)
		if #sg>0 then
