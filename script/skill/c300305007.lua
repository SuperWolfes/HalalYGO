...
function s.cfilter(c)
	return c:IsSetCard(SET_HERO) and (c:IsType(TYPE_NORMAL) or c:IsType(TYPE_FUSION))
end
--Returnable to hand
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	local b4=#g>0 and g:FilterCount(Card.IsAttribute,nil,ATTRIBUTE_WATER)>0 and Duel.IsPlayerCanDraw(tp,1)
	local b5=#g>0 and g:FilterCount(Card.IsAttribute,nil,ATTRIBUTE_LIGHT)>0
             and Duel.IsExistingMatchingCard(s.posfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil)
	...
	elseif op==3 then
		local rg=Duel.GetMatchingGroup(s.thfilter,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
