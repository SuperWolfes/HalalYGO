...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.spfilter),tp,LOCATION_HAND_DECK_RP,0,1,1,nil,e,tp)
	if tc then
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Release(g,REASON_COST)>0 then
		...
		local g1=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.thfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,1,nil,49064413)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
		local g2=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.thfilter),tp,LOCATION_DECK|LOCATION_REST,0,1,1,nil,94377247)
		g1:Merge(g2)
