...
function s.tgfilter(c,tp)
	return c:IsLevel(2) and c:IsRace(RACE_TOXIC) and c:IsAbleToGraveAsCost() and Duel.IsExistingMatchingCard(s.sfilter,tp,LOCATION_DECK+LOCATION_REST+LOCATION_REMOVED,0,1,nil)
end
function s.sfilter(c)
	return c:IsCode(31076103,4081094,78697395) and not c:IsForbidden() and (c:IsFaceup() or c:IsLocation(LOCATION_REST+LOCATION_DECK))
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.SendtoGrave(g,REASON_COST)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.sfilter),tp,LOCATION_DECK+LOCATION_REST+LOCATION_REMOVED,0,1,1,nil)
		if #sg>0 then
