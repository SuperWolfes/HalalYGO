...
function Debug.AddMaximumCard(player,center,left,right)
	...
	c:RegisterFlagEffect(FLAG_MAXIMUM_CENTER_PREONFIELD,RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD-RESET_TORESTING-RESET_LEAVE,0,1)
	...
	for _,tc in ipairs({l,r}) do
		...
		tc:RegisterFlagEffect(FLAG_MAXIMUM_SIDE_PREONFIELD,RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD-RESET_TORESTING-RESET_LEAVE,0,1)
	end
end
--Maximum Summon
...
function Maximum.Operation(mats)
	...
	return function(e,tp,eg,ep,ev,re,r,rp,c,sg,og)
		...
		Duel.SendtoResting(g,REASON_RULE)
	end
end
...
function Card.AddCannotBeDestroyedEffect(c,ctype,reset,rc,condition,properties)
	...
	elseif ctype==TYPE_ACTIONAL then e1:SetDescription(3069)
...
--Rush cost utilities
-- p: player that mill
-- num: number of card send from the top to the rp
