...
function s.tgfilter(c,tp)
	return c:IsSetCard(0x111) and c:IsFaceup() and Duel.IsExistingMatchingCard(s.atkfilter,tp,LOCATION_REST,0,1,nil,c:GetLevel())
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(s.atkfilter,tp,LOCATION_REST,0,nil,tc:GetLevel())
...
function s.thfilter(c)
	return c:IsActional() and c:IsSetCard(0x111) and c:IsAbleToHand()
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp) and s.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(s.thfilter,tp,LOCATION_REST,0,1,e:GetHandler(),nil) end
