...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		if diff==0 then
			return Duel.IsExistingMatchingCard(Card.IsActionalTrap,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,e:GetHandler())
		elseif diff<=3 then
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if diff==0 then
		...
		local g=Duel.SelectMatchingCard(tp,Card.IsActionalTrap,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,2,2,e:GetHandler())
		if #g>0 then
