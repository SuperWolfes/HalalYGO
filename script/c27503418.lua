...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_REMOVE,nil,2,tp,LOCATION_MZONE+LOCATION_REST)
...
function s.filter1(c,e,tp,tc,lv)
	...
	return rlv>0 and c:IsType(TYPE_SYNCHRO) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SYNCHRO,tp,false,false)
		...
		and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,c,tp,rlv)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.NegateAttack() and Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_EXTRA,0,1,nil,e,tp,tc,tc:GetLevel())
		...
		local g2=Duel.SelectMatchingCard(tp,s.filter2,tp,LOCATION_MZONE+LOCATION_REST,0,1,1,nil,tp,lv)
		g2:AddCard(tc)
