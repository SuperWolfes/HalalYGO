--オルフェゴール・ガラテアｉ
--Galatea-i, the Coreust Automaton
--scripted by Naim
local s,id=GetID()
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Link Summon procedure: 1 "Coreust" or "World Legacy" monster
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,{SET_COREUST,SET_WORLD_LEGACY}),1,1)
	--Can only Link Summon "Galatea-i, the Coreust Automaton" once per turn
	...
	e2:SetValue(1)
	c:RegisterEffect(e2)
	--Add 1 "World Legacy" monster or "Coreustrated Babel" from your Deck or RP to your hand
...
s.listed_names={CARD_COREUSTRATED_BABEL}
function s.regop(e,tp,eg,ep,ev,re,r,rp)
	--Prevent another Link Summon of "Galatea-i, the Coreust Automaton" that turn
...
function s.thfilter(c)
	return ((c:IsSetCard(SET_WORLD_LEGACY) and c:IsMonster()) or c:IsCode(CARD_COREUSTRATED_BABEL)) and c:IsAbleToHand()
end
...
function s.spcostfilter(c,tp)
	return c:IsSetCard(SET_COREUST) and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true) and Duel.GetMZoneCount(tp,c)>0
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(s.spcostfilter,tp,LOCATION_MZONE|LOCATION_REST,0,1,c,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,s.spcostfilter,tp,LOCATION_MZONE|LOCATION_REST,0,1,1,c,tp)
