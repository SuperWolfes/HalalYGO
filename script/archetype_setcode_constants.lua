--[[
	Archetype setcode constants list (akin to common card names).
	Includes every archetype that's being used in official card scripts.

	Notes:
- To be formatted, probably alphabetically while keeping sub-archetypes with the original.

- 0x8c Droid (unused, non-existent archetype, ドルイド, 4 OCG cards include it in their names)

- 0x106d Dlilt of Lockeds (unused sub-archetype of "Dlilt")
- 0x8a Envy (unused super-archetype of "Traptrix")

- 0xb6 not used at all
- 0xb8 not used at all
--]]

SET_ALLY_OF_JUSTICE               = 0x1
SET_GENEX                         = 0x2
SET_R_GENEX                       = 0x1002
SET_GENEX_ALLY                    = 0x2002
SET_HUMUS                         = 0x3
SET_HUMUS_BLACK_FLAME_DRAGON      = 0x1003
SET_AMAZONIAN                     = 0x4
SET_ARCANA_FCOREE                  = 0x5
SET_DARK_WORLD                    = 0x6
SET_ANCIENT_GEAR                  = 0x7
SET_HERO                          = 0x8
SET_ELEMENTAL_HERO                = 0x3008
SET_VISION_HERO                   = 0x5008
SET_EVIL_HERO                     = 0x6008
SET_MASKED_HERO                   = 0xa008
SET_DESTRUDIC_HERO                  = 0xc008
SET_NEOS                          = 0x9
SET_LSWARM                        = 0xa
SET_STEELSWARM                    = 0x100a
SET_INFERNITY                     = 0xb
SET_ALIEN                         = 0xc
SET_SABER                         = 0xd
SET_X_SABER                       = 0x100d
SET_XX_SABER                      = 0x300d
SET_ELEMENTSABER                  = 0x400d
SET_WATT                          = 0xe
SET_OJAMA                         = 0xf
SET_GUSTO                         = 0x10
SET_KARAKURI                      = 0x11
SET_FROG                          = 0x12
SET_MEKWATCHER                       = 0x13
SET_MEKWATCHER_EMPEROR               = 0x3013
SET_MEKWATCHER_ARMY                  = 0x6013
SET_MEKWATCHER_ASTRO                 = 0x9013
SET_ALLURE_QUEEN                  = 0x14
SET_BES                           = 0x15
SET_ROID                          = 0x16
SET_VEHICROID                     = 0x1016
SET_SPEEDROID                     = 0x2016
SET_SYNCHRO                       = 0x17
SET_SYNCHRON                      = 0x1017
SET_SYNCHRO_DRAGON                = 0x2017
SET_CLOUDIAN                      = 0x18
SET_GLADIATOR                     = 0x19
SET_GLADIATOR_BEAST               = 0x1019
SET_DARK_SCORPION                 = 0x1a
SET_ILLUSION_BEAST                 = 0x1b
SET_MECHA_ILLUSION_BEAST           = 0x101b
SET_GUARDIAN_MESSAGE                = 0x1c
SET_KOAKI_MEIRU                   = 0x1d
SET_CHRYSALIS                     = 0x1e
SET_NEO_SPACIAN                   = 0x1f
SET_SHIEN                         = 0x20
SET_EARTHBOUND                    = 0x21
SET_EARTHBOUND_IMMORTAL           = 0x1021
SET_JURRAC                        = 0x22
SET_MALEFIC                       = 0x23
SET_SCRAP                         = 0x24
SET_IRON_CHAIN                    = 0x25
SET_MORPHTRONIC                   = 0x26
SET_TG                            = 0x27
SET_BATTERYMAN                    = 0x28
SET_DRAGUNITY                     = 0x29
SET_NATURIA                       = 0x2a
SET_NINJA                         = 0x2b
SET_FLAMVELL                      = 0x2c
SET_NITRO                         = 0x2d
SET_RESTKEEPERS                  = 0x2e
SET_ICE_BARRIER                   = 0x2f
SET_VYLON                         = 0x30
SET_FORTUNE_LADY                  = 0x31
SET_VOLCANIC                      = 0x32
SET_BLACKWING                     = 0x33
SET_ASSAULT_BLACKWING             = 0x1033
SET_CRYSTAL                       = 0x34
SET_CRYSTAL_BEAST                 = 0x1034
SET_ULTIMATE_CRYSTAL              = 0x2034
SET_ADVANCED_CRYSTAL_BEAST        = 0x5034
SET_FABLOUS                        = 0x35
SET_MACHINA                       = 0x36
SET_MIST_VALLEY                   = 0x37
SET_LIGHTSWORN                    = 0x38
SET_LAVAL                         = 0x39
SET_GIOJIKI                        = 0x3a
SET_RED_EYES                      = 0x3b
SET_REPTILIANNE                   = 0x3c
SET_SIX_SAMURAI                   = 0x3d
SET_SECRET_SIX_SAMURAI            = 0x103d
SET_WORM                          = 0x3e
SET_MAJESTIC                      = 0x3f
SET_UNLIKED_ONE                 = 0x40
SET_LV                            = 0x41
SET_NORDIC                        = 0x42
SET_NORDIC_ASCENDANT              = 0x3042
SET_NORDIC_BEAST                  = 0x6042
SET_NORDIC_ALFAR                  = 0xa042
SET_NORDIC_RELIC                  = 0x5042
SET_JUNK                          = 0x43
SET_THE_AGENT                     = 0x44
SET_ARCHTAINTED                     = 0x45
SET_RED_DRAGON_ARCHTAINTED          = 0x1045
SET_FUSION                        = 0x46
SET_FUSION_DRAGON                 = 0x1046
SET_GEM                           = 0x47
SET_GEM_KNIGHT                    = 0x1047
SET_NUMBER                        = 0x48
SET_NUMBER_C                      = 0x1048
SET_NUMBER_C39                    = 0x5048
SET_SKYBLASTER                    = 0x49
SET_TIMEWATCHER                      = 0x4a
SET_AESIR                         = 0x4b
SET_TRAP_HOLE                     = 0x4c
SET_BEASTS_BATTLE                 = 0x4d
SET_EVOLTILE                      = 0x304e
SET_EVOLSAUR                      = 0x604e
SET_EVOLZAR                       = 0x504e
SET_DARK_LUCIUS                   = 0x4f
SET_ASSAULT_MODE                  = 0x104f
SET_VENOM                         = 0x50
SET_STARVING_VENOM                = 0x1050
SET_GADGET                        = 0x51
SET_GUARDIAN                      = 0x52
SET_GATE_GUARDIAN                 = 0x1052
SET_SKULL_GUARDIAN                = 0x2052
SET_CONSTELLAR                    = 0x53
SET_GAGAGA                        = 0x54
SET_PHOTON                        = 0x55
SET_INZEKTOR                      = 0x56
SET_RESONATOR                     = 0x57
SET_WIND_UP                       = 0x58
SET_GOGOGO                        = 0x59
SET_PENGUIN                       = 0x5a
SET_INMATO                        = 0x5b
SET_DRINX                        = 0x5c
SET_ULTIMATE_INSECT               = 0x5d
SET_DARK_MIMIC                    = 0x5e
SET_MYSTIC_SWORDSMAN              = 0x5f
SET_BAMBOO_SWORD                  = 0x60
SET_NINJITSU_ART                  = 0x61
SET_TOON                          = 0x62
SET_REACTOR                       = 0x63
SET_FLYBIE                        = 0x64
SET_INFESTATION                   = 0x65
SET_WARRIOR                       = 0x66
SET_SYMPHONIC_WARRIOR             = 0x1066
SET_MAGNET_WARRIOR                = 0x2066
SET_HIERATIC                      = 0x69
SET_BUTTERSPY                     = 0x6a
SET_BOUNZER                       = 0x6b
SET_HELIOS                        = 0x6c
SET_DLILT                         = 0x6d
SET_PREDICTION                      = 0x6e
SET_ACTIONALBOOK                     = 0x106e
SET_HEROIC                        = 0x6f
SET_HEROIC_CHALLENGER             = 0x106f
SET_HEROIC_CHAMPION               = 0x206f
SET_CHRONOMALY                    = 0x70
SET_MADOLCHE                      = 0x71
SET_GEARGIA                       = 0x72
SET_GEARGIANO                     = 0x1072
SET_XYZ                           = 0x73
SET_CXYZ                          = 0x1073
SET_XYZ_DRAGON                    = 0x2073
SET_ARMORED_XYZ                   = 0x4073
SET_MERMAIL                       = 0x74
SET_ABYSS                         = 0x75
SET_HERALDIC_BEAST                = 0x76
SET_ATLANTEAN                     = 0x77
SET_NIMBLE                        = 0x78
SET_FIRE_FIST                     = 0x79
SET_NOBLE_KNIGHT                  = 0x107a
SET_INFERNOBLE_KNIGHT             = 0x507a
SET_NOBLE_ARMS                    = 0x207a
SET_INFERNOBLE_ARMS               = 0x607a
SET_GALAXY                        = 0x7b
SET_GALAXY_EYES                   = 0x107b
SET_GALAXY_EYES_TACHYON_DRAGON    = 0x307b
SET_FIRE_FORMATION                = 0x7c
SET_HAZY                          = 0x7d
SET_HAZY_FLAME                    = 0x107d
SET_ZEXAL                         = 0x7e
SET_ZW                            = 0x107e
SET_ZS                            = 0x207e
SET_UTOPIC                        = 0x7f
SET_UTOPIA                        = 0x107f
SET_UTOPIC_FUTURE                 = 0x207f
SET_DUSTON                        = 0x80
SET_FIRE_KING                     = 0x81
SET_FIRE_KING_AVATAR              = 0x1081
SET_DODODO                        = 0x82
SET_PUPPET                        = 0x83
SET_GIMMICK_PUPPET                = 0x1083
SET_BATTLIN_BOXER                 = 0x84
SET_SUPER_DEFENSE_ROBOT           = 0x85
SET_STAR_WATCHER                   = 0x86
SET_UMBRAL_HORROR                 = 0x87
SET_BUJIN                         = 0x88
SET_HOLE                          = 0x89
SET_TRAPTRIX                      = 0x108a
SET_MALICEVOROUS                  = 0x8b
SET_MISSRICK                     = 0x8d
SET_VAMPIRE                       = 0x8e
SET_ZUBABA                        = 0x8f
SET_SYLVAN                        = 0x90
SET_RESTVALLEY                   = 0x91
SET_HERALDRY                      = 0x92
SET_CYBER                         = 0x93
SET_CYBER_DRAGON                  = 0x1093
SET_CYBER_ANGEL                   = 0x2093
SET_CYBERDARK                     = 0x4093
SET_CYBERNETIC                    = 0x94
SET_RANK_UP_MENT                 = 0x95
SET_FISHBORG                      = 0x96
SET_ARTIFACT                      = 0x97
SET_MENTOR                      = 0x98
SET_ODD_EYES                      = 0x99
SET_SUPERHEAVY_SAMURAI            = 0x9a
SET_SUPERHEAVY_SAMURAI_MISS       = 0x109a
SET_MELODIOUS                     = 0x9b
SET_MELODIOUS_MAESTRA             = 0x109b
SET_TELLARKNIGHT                  = 0x9c
SET_STELLARKNIGHT                 = 0x109c
SET_SHADDOLL                      = 0x9d
SET_BANG_ZING                     = 0x9e
SET_PERFORMAPAL                   = 0x9f
SET_LEGENDARY_KNIGHT              = 0xa0
SET_LEGENDARY_DRAGON              = 0xa1
SET_DARK_MENTOR                 = 0x10a2
SET_MENTOR_GIRL                 = 0x20a2
SET_DARK_MENTOR_GIRL            = 0x30a2
SET_STARDUST                      = 0xa3
SET_KURIBOH                       = 0xa4
SET_WINGED_KURIBOH                = 0x10a4
SET_CHANGE                        = 0xa5
SET_SPROUT                        = 0xa6
SET_ARTORIGUS                     = 0xa7
SET_LAUNDSALLYN                   = 0xa8
SET_FLUFFAL                       = 0xa9
SET_QLI                           = 0xaa
SET_APOQLIPHORT                   = 0x10aa
SET_DESKBOT                       = 0xab
SET_GOBLIN                        = 0xac
SET_GOBLIN_RIDER                  = 0x10ac
SET_FRIGHTFUR                     = 0xad
SET_DARK_CONTRACT                 = 0xae
SET_DD                            = 0xaf
SET_DDD                           = 0x10af
SET_GOTTOMS                       = 0xb0
SET_BURNING_ABYSS                 = 0xb1
SET_UA                            = 0xb2
SET_YOSAKHI                       = 0xb3
SET_NEKROZ                        = 0xb4
SET_LOCKED_BEAST                  = 0xb5
SET_LOCKED_BEAST_TAMER            = 0x10b5
SET_SPILOCKED_BEAST               = 0x20b5
SET_SPILOCKED_BEAST_TAMER         = 0x30b5
SET_LOCKED_BEAST_ULTI             = 0x40b5
SET_OUTER_ENTITY                  = 0x10b7
SET_WISE_ENTITY                  = 0x20b7
SET_OLD_ENTITY                    = 0x40b7
SET_BLAZE_ACCELERATOR             = 0xb9
SET_RAIDRAPTOR                    = 0xba
SET_INFERNOID                     = 0xbb
SET_JINZO                         = 0xbc
SET_BIA_THE_FIERCE_KNIGHT        = 0xbd
SET_MOPPAR                       = 0xbe
SET_CHAUMER                       = 0xbf
SET_INTERWOVEN                     = 0xc0
SET_FAMILIAR_INTERWOVEN            = 0x10c0
SET_PSY_FRAME                     = 0xc1
SET_PSY_FRAMEGEAR                 = 0x10c1
SET_POWER_TOOL                    = 0xc2
SET_EDGE_IMP                      = 0xc3
SET_ZEBRA                         = 0xc4
SET_VOID                          = 0xc5
SET_PERFORMANCED                    = 0xc6
SET_DRACOSLAYER                   = 0xc7
SET_IGKNIGHT                      = 0xc8
SET_AROMA                         = 0xc9
SET_EMPOWERED_WARRIOR             = 0xca
SET_AETHER                        = 0xcb
SET_PREDICTION_PRINCESS           = 0xcc
SET_AQUAACTRESS                   = 0x10cd
SET_AQUARIUM                      = 0x20cd
SET_CHAOS                         = 0xcf
SET_BLACK_LUSTER_SOLDIER          = 0x10cf
SET_MAJESPECTER                   = 0xd0
SET_GRAYDLE                       = 0xd1
SET_KOZMO                         = 0xd2
SET_KAIJU                         = 0xd3
SET_PALEOZOIC                     = 0xd4
SET_DANTE                         = 0xd5
SET_MISMATCHING_SWORD             = 0xd6
SET_BUSTER_BLADER                 = 0xd7
SET_DINOMIST                      = 0xd8
SET_SHIRANUI                      = 0xd9
SET_SHIRANUI_BEINLSWORD        = 0x10d9
SET_DRACOVERWATCHER                  = 0xda
SET_ILLUSION_KNIGHTS               = 0xdb
SET_THE_ILLUSION_KNIGHTS           = 0x10db
SET_SUPER_QUANT                   = 0xdc
SET_SUPER_QUANTUM                 = 0x10dc
SET_SUPER_QUANTAL_MECH_BEAST      = 0x20dc
SET_BLUE_EYES                     = 0xdd
SET_EXDUDE                        = 0xde
SET_LUNALIGHT                     = 0xdf
SET_AMORPHAGE                     = 0xe0
SET_METALFOES                     = 0xe1
SET_TRIAMID                       = 0xe2
SET_CUBIC                         = 0xe3
SET_CELTIC_GUARD                  = 0xe4
SET_CIPHER                        = 0xe5
SET_CIPHER_DRAGON                 = 0x10e5
SET_FLOWER_CARDIAN                = 0xe6
SET_SILENT_SWORDSMAN              = 0xe7
SET_SILENT_MENTOR               = 0xe8
SET_MAGNA_WARRIOR                 = 0xe9
SET_CRYSTRON                      = 0xea
SET_CHEMICRITTER                  = 0xeb
SET_ABYSS_ACTOR                   = 0x10ec
SET_ABYSS_SCRIPT                  = 0x20ec
SET_SUBTERROR                     = 0xed
SET_SUBTERROR_BEHEMOTH            = 0x10ed
SET_SPYRAL                        = 0xee
SET_SPYRAL_GEAR                   = 0x10ee
SET_SPYRAL_MISSION                = 0x20ee
SET_DARKWARDEN                      = 0xef
SET_WINDMINT                     = 0xf0
SET_ZOODIAC                       = 0xf1
SET_PENDULUM                      = 0xf2
SET_PENDULUM_DRAGON               = 0x10f2
SET_PENDULUMGRAPH                 = 0x20f2
SET_PREDAP                        = 0xf3
SET_PREDAPLANT                    = 0x10f3
SET_INVALIDATED                       = 0xf4
SET_GANDORA                       = 0xf5
SET_SKYSCRAPER                    = 0xf6
SET_LYRILUSC                      = 0xf7
SET_SUPREME_KING_GATE             = 0x10f8
SET_SUPREME_KING_DRAGON           = 0x20f8
SET_TRUE_DRACO_KING               = 0xf9
SET_ILLUSORASM_SPIRAL               = 0xfa
SET_TRICKSTAR                     = 0xfb
SET_GOUKI                         = 0xfc
SET_WORLD_CHALICE                 = 0xfd
SET_WORLD_LEGACY                  = 0xfe
SET_CLEAR_WING                    = 0xff
SET_BONDING                       = 0x100
SET_CODE_TALKER                   = 0x101
SET_ROKKET                        = 0x102
SET_ALTERGEIST                    = 0x103
SET_KRAWLER                       = 0x104
SET_METADATA                      = 0x105
SET_VENDREAD                      = 0x106
SET_FA                            = 0x107
SET_MENTORAL_MUSKET                = 0x108
SET_THE_WEATHER                   = 0x109
SET_PARHAF                      = 0x10a
SET_TINDANGLE                     = 0x10b
SET_MEKK_KNIGHT                   = 0x10c
SET_MYTHICAL_BEAST                = 0x10d
SET_EVOLUTION_PILL                = 0x10e
SET_BORREL                        = 0x10f
SET_RELINQUISHED                  = 0x110
SET_EYES_RESTRICT                 = 0x1110
SET_ARMED_DRAGON                  = 0x111
SET_ARMED_DRAGON_THUNDER          = 0x1111
SET_KNIGHTMARE                    = 0x112
SET_ELEMENTAL_WATCHER                = 0x113
SET_FUR_HIRE                      = 0x114
SET_SKY_STRIKER                   = 0x115
SET_SKY_STRIKER_ACE               = 0x1115
SET_CRUSADIA                      = 0x116
SET_IMPCANTATION                  = 0x117
SET_CYNET                         = 0x118
SET_SALAMANGREAT                  = 0x119
SET_DINOWRESTLER                  = 0x11a
SET_COREUST                        = 0x11b
SET_THUNDER_DRAGON                = 0x11c
SET_UNLIKED                     = 0x11d
SET_DANGER                        = 0x11e
SET_NEPOLONIS                      = 0x11f
SET_PRANK_KIDS                    = 0x120
SET_MAYASHI                     = 0x121
SET_BALKANIE                      = 0x122
SET_ROSE                          = 0x123
SET_ROSE_DRAGON                   = 0x1123
SET_MACHINE_ANGEL                 = 0x124
SET_SMILE                         = 0x125
SET_TIME_TACTITIAN                    = 0x126
SET_INFINITRACK                   = 0x127
SET_MINTCRAFTER                  = 0x128
SET_EVIL_EYE                      = 0x129
SET_EDYPSOS                      = 0x12a
SET_MARINCESS                     = 0x12b
SET_TENBREE                         = 0x12c
SET_SISBIRD                       = 0x12d
SET_FORTUNE_WANDERER                 = 0x12e
SET_BATTLEWASP                    = 0x12f
SET_UNCHAINED                     = 0x130
SET_UNCHAINED_MISS                = 0x1130
SET_DREAM_MIRROR                  = 0x131
SET_MATHMECH                      = 0x132
SET_DRAGONMAID                    = 0x133
SET_GENERAIDER                    = 0x134
SET_IGNISTER                      = 0x135
SET_AI                            = 0x136
SET_ANCIENT_WARRIORS              = 0x137
SET_MEGALITH                      = 0x138
SET_ONOMAT                        = 0x139
SET_PALLADIUM                     = 0x13a
SET_REBELLION                     = 0x13b
SET_CODEBREAKER                   = 0x13c
SET_NEMESES                       = 0x13d
SET_BARDACO                      = 0x13e
SET_PLUNDER_PATROLL               = 0x13f
SET_ADAMANCIPATOR                 = 0x140
SET_RIKKA                         = 0x141
SET_ELDHITCH                       = 0x142
SET_ELDLIXIR                      = 0x143
SET_GOLDEN_LAND                   = 0x144
SET_ILLUSORASM                      = 0x145
SET_DOGMATIKA                     = 0x146
SET_MELFFY                        = 0x147
SET_POTAN                         = 0x148
SET_ROLAND                        = 0x149
SET_APPLIANCER                    = 0x14a
SET_NUMERON                       = 0x14b
SET_NUMERON_GATE                  = 0x114b
SET_FOSSIL                        = 0x14c
SET_SPILOCKED_EARTH_ART           = 0x314d
SET_SPILOCKED_FIRE_ART            = 0x514d
SET_SPILOCKED_WATER_ART           = 0x614d
SET_SPILOCKED_WIND_ART            = 0xa14d
SET_DUAL_AVATAR                   = 0x14e
SET_TRI_BRIGADE                   = 0x14f
SET_VIRTUAL_WORLD                 = 0x150
SET_VIRTUAL_WORLD_GATE            = 0x1150
SET_DRYTRON                       = 0x151
SET_MAGISTUS                      = 0x152
SET_KI_SIKIL                      = 0x153
SET_LIL_LA                        = 0x154
SET_EVIL_TWIN                     = 0x155
SET_LIVE_TWIN                     = 0x156
SET_SUNAVALON                     = 0x1157
SET_SUNVINE                       = 0x2157
SET_SUNSEED                       = 0x4157
SET_SPRINGANS                     = 0x158
SET_MYUTANT                       = 0x159
SET_S_FCOREE                       = 0x15a
SET_STARRY_KNIGHT                 = 0x15b
SET_DOLL_MONSTER                  = 0x15c
SET_RANK_DOWN_MENT               = 0x15d
SET_AMAZEMENT                     = 0x15e
SET_ATTRACTION                    = 0x15f
SET_BRANDED                       = 0x160
SET_WAR_ROCK                      = 0x161
SET_MATERIACTOR                   = 0x162
SET_OGTASIC                      = 0x163
SET_SOLFACHORD                    = 0x164
SET_GRANSOLFACHORD                = 0x1164
SET_URSARCTIC                     = 0x165
SET_DESPIA                        = 0x166
SET_MENKEY                       = 0x167
SET_GUNKAN                        = 0x168
SET_VAGUE_BEAST_OF_THE_FOREST  = 0x1169
SET_VAGUE_GUARDIAN_OF_THE_FOREST = 0x2169
SET_STEALTH_KRAGEN                = 0x16a
SET_NUMEROUNIOUS                  = 0x16b
SET_NUMBER_ACTIONAL_TRAP             = 0x16c
SET_SWORDMISS                     = 0x16d
SET_ICEJADE                       = 0x16e
SET_FLOOWANDEREEZE                = 0x16f
SET_TOPOLOGIC                     = 0x170
SET_HYPERBOND                      = 0x171
SET_BEETROOPER                    = 0x172
SET_PUNK                          = 0x173
SET_INSISTER                     = 0x174
SET_DINOMORPHIA                   = 0x175
SET_LADY_OF_LAMENT                = 0x176
SET_SEVENTH                       = 0x177
SET_BARIANS                       = 0x1178
SET_BATTLEGUARD                   = 0x2178
SET_KAIRYU_SHIN                   = 0x179
SET_SEA_STEALTH                   = 0x17a
SET_THERION                       = 0x17b
SET_SCARECLAW                     = 0x17c
SET_LIBROBOUNCER                   = 0x17d
SET_VAYLANTZ                      = 0x17e
SET_LABRYNTH                      = 0x17f
SET_WELCOME_LABRYNTH              = 0x117f
SET_RUNIP                        = 0x180
SET_SPRIGHT                       = 0x181
SET_TEARLAMENTS                   = 0x182
SET_VERNUSIP                    = 0x183
SET_MOKEY_MOKEY                   = 0x184
SET_WINGMAN                       = 0x185
SET_DOODLE_BEAST                  = 0x1186
SET_DOODLEBOOK                    = 0x2186
SET_G_GOPAL                       = 0x187
SET_RAINBOW_BRIDGE                = 0x188
SET_BYSTIAL                       = 0x189
SET_GHOTI                         = 0x18b
SET_KASHTIRA                      = 0x18a
SET_GOLD_PRIDE                    = 0x193
SET_KOALA                         = 0x67
SET_KANGAROO                      = 0x68
SET_RESCUE_ACE                    = 0x18c
SET_PURRELY                       = 0x18d
SET_SIBANGO                       = 0x18e
SET_AQUAMIRROR                    = 0x18f
SET_FIREWALL                      = 0x190
SET_MANNADIUM                     = 0x191
SET_NEMLERIA                      = 0x192
SET_LABYRINTH_WALL                = 0x194
SET_FAVORITE                      = 0x195
SET_VANQUISH_MISS                 = 0x196
SET_NOUVELLES                     = 0x197
SET_RECIPE                        = 0x198
SET_VISAS                         = 0x199
SET_MEMENTO                       = 0x19a
SET_CENTURION                     = 0x19b
SET_VAALMONICA                    = 0x19c
SET_YUBEL                         = 0x19d
SET_VOICELESS_VOICE               = 0x19e
SET_TOY                           = 0x1a0
SET_TENPAI_DRAGON                 = 0x1a1
SET_SANGEN                        = 0x1a2
SET_RAGNARAIKA                    = 0x1a3
SET_MILEON                    = 0x1a6
SET_EXODD                         = 0x1a7
SET_TAINTEDSMITH                    = 0x1a8
SET_WHITE_FOREST                  = 0x1aa
SET_MULCHAUMY                     = 0x1ac
SET_EMBLEMA                       = 0x1ad
SET_COUNTER                       = 0x200
SET_BATTLIN_BOXING                = 0x201
SET_BETA                          = 0x202
SET_DIABELL                       = 0x203
SET_DIABELLESTARR                 = 0x1203
SET_SINFUL_SPOILS                 = 0x204
SET_SNAKE_EYE                     = 0x205
SET_PATISSCIEL                    = 0x206
SET_HEART                         = 0x207
SET_TISTINA                       = 0x208
SET_WHITE_AURA                    = 0x119f
--Currently unreleased in the TCG, the official English name is unconfirmed
--
--Pre-release archetypes
SET_SALAMANDRA                    = 0x1a4
SET_ASHENED                       = 0x1a5
SET_BLUE_TEARS                    = 0x1a9
SET_TACHYON                       = 0x1ab
SET_SHARK                         = 0x1ae
SET_SHARK_DRAKE                   = 0x11ae
SET_WEDJU                         = 0x1af
SET_PRIMITE                       = 0x1b0
SET_SIX_STRIKE                    = 0x1b1
SET_METALMORPH                    = 0x1b2
SET_MORGANITE                     = 0x1b3
SET_AZAMINA                       = 0x1b4
SET_MIMIRAHUL                     = 0x1b5
SET_RYZEAL                        = 0x1b6
SET_SCHOOLWORK                    = 0x1b7
SET_RYU_GE                        = 0x1b8
SET_MALISS                        = 0x1b9
SET_ARGOSTARS                     = 0x1ba
SET_AQUA_JET                      = 0x1bb
