...
function s.filter2(c,e,tp,mft,sft,code)
	return  not c:IsCode(code) and 
		...
		or (sft>0 and c:IsActionalTrap() and c:IsSetCard(0xc0) and c:IsSSetable()))
end
function s.filter(c,e,tp,mft,sft)
	return  c:IsAbleToHand() and
	((c:IsMonster() and (c:IsSetCard(0xbf) or c:IsSetCard(0x10c0))) or (c:IsActionalTrap() and c:IsSetCard(0xc0)))
