...
function Card.IsActional(c)
	return c:IsType(TYPE_ACTIONAL)
end
...
function Card.IsActionalTrap(c)
	return c:IsType(TYPE_ACTIONAL+TYPE_TRAP)
end

function Card.IsLockedMonster(c)
	local tp=TYPE_LOCKED+TYPE_MONSTER
	return c:GetType() & tp == tp
end

function Card.IsLockedActional(c)
	local tp=TYPE_LOCKED+TYPE_ACTIONAL
	return c:GetType() & tp == tp
end
...
function Card.IsLinkActional(c)
	local tp=TYPE_LINK+TYPE_ACTIONAL
	return c:GetType() & tp == tp
end
...
function Card.RegisterEffect(c,e,forced,...)
	...
	--8 == 511310036 - access to Allure Queen effects that activate by sending themselves to RP
