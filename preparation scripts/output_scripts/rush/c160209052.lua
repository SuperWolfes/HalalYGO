--斬神キバツ
--Kibatsu the Cutting Edge Being
--scripted by YoshiDuels
local s,id=GetID()
function s.initial_effect(c)
	--Excavate the top 3 cards and send 1 to the RP
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if g:IsExists(Card.IsMonster,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,1)) then
		...
		if Duel.SendtoRest(sg,REASON_EFFECT)>0 and sg:GetFirst():IsCode(id) and #dg>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
