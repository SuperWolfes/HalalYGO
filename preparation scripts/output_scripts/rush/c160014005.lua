...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg1=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.tdfilter),tp,0,LOCATION_REST,3,3,nil)
	if #dg1==0 then return end
	Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_TODECK)
	local dg2=Duel.SelectMatchingCard(1-tp,aux.RestValleyFilter(s.tdfilter),tp,LOCATION_REST,0,3,3,nil)
