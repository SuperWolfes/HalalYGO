...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct then
		...
		if ct:IsMonster() and ct:IsRace(RACE_CYBERSE) and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_REST,0,1,nil) and ct2>0 and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
			local g=Duel.SelectMatchingCard(tp,s.thfilter,tp,LOCATION_REST,0,1,ct2,nil)
			if #g>0 then
