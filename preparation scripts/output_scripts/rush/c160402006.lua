...
function s.initial_effect(c)
	--Special summon 1 level 6 mentor monster from hand
...
function s.cfilter(c)
	return (c:IsRace(RACE_MENTOR) or c:IsRace(RACE_WARRIOR)) and c:IsAbleToDeckOrExtraAsCost()
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REST,0,2,nil) end
end
function s.filter(c,e,tp)
	return (c:IsRace(RACE_MENTOR) or c:IsRace(RACE_WARRIOR)) and c:IsLevel(6) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
