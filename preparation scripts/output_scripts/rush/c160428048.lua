...
function s.initial_effect(c)
	--Return to hand an attack target and a monster from RP to the hand
...
function s.psyfilter(c)
	return c:IsFaceup() and c:IsLevelAbove(7) and (c:IsRace(RACE_MENTAL) or c:IsRace(RACE_OMEGAMENTAL))
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToBattle() and Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_HAND)
		...
		and Duel.IsExistingMatchingCard(s.thfilter,tp,LOCATION_REST,0,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
