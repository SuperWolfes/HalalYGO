...
function s.initial_effect(c)
	...
	e2:SetCategory(CATEGORY_TOHAND+CATEGORY_TOREST)
...
function s.thfilter(c)
	return c:IsActionalTrap() and c:IsSetCard(0x7c) and c:IsAbleToHand()
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(s.thfilter,tp,LOCATION_ONFIELD+LOCATION_REST,0,1,nil)
		and Duel.IsExistingMatchingCard(s.gyfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,s.thfilter,tp,LOCATION_ONFIELD+LOCATION_REST,0,1,1,nil)
