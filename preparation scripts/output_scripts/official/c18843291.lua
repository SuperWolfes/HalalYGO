...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Send "Lightsworn" monsters with different Types from your Deck to the RP
...
function s.tgfilter(c)
	return c:IsSetCard(SET_LIGHTSWORN) and c:IsMonster() and c:IsAbleToRest()
end
...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=aux.SelectUnselectGroup(g,e,tp,1,ct,aux.dpcheck(Card.GetRace),1,tp,HINTMSG_TOREST)
	if #tg>0 then
...
function s.toptgcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_REST,0,1,ct,nil)
