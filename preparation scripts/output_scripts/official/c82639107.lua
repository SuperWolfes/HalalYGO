...
function s.tgfilter(c)
	return c:IsSetCard(SET_NEO_SPACIAN) and c:IsAbleToRest()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_TOREST,nil,2,tp,LOCATION_HAND|LOCATION_DECK)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(sg,REASON_EFFECT)==2 and sg:FilterCount(Card.IsLocation,nil,LOCATION_REST)==2
