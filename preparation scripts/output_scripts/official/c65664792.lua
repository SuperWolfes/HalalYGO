...
function s.initial_effect(c)
	--Shuffle 1 "Flybie Lady Sisters" into the Deck and Special Summon 3 "Flybie" monsters
	...
	c:RegisterEffect(e1)
	--Add 1 "Flybie" monster from the Deck to the hand
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(SET_FLYBIE) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.spcheck(sg,e,tp,mg)
	return sg:GetClassCount(Card.GetOriginalCodeRule)==#sg and sg:GetClassCount(Card.GetLocation)==#sg
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.RestValleyFilter(s.spfilter),tp,LOCATION_HAND|LOCATION_DECK|LOCATION_REST,0,nil,e,tp)
	...
	--Cannot Special Summon non-WIND monsters
...
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return c:IsPreviousLocation(LOCATION_ONFIELD) and (rp==1-tp or (rp==tp and re:GetHandler():IsSetCard(SET_FLYBIE))) and c:IsPreviousControler(tp)
end
function s.thfilter(c)
	return c:IsMonster() and c:IsSetCard(SET_FLYBIE) and c:IsAbleToHand()
end
