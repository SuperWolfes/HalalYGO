...
function s.initial_effect(c)
	--Send the top card of your Deck to the RP, then, if it is a WATER monster, draw 1 card
...
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.DiscardDeck(tp,1,REASON_EFFECT)==1 then
		...
		if sc:IsAttribute(ATTRIBUTE_WATER) and sc:IsLocation(LOCATION_REST) and Duel.IsPlayerCanDraw(tp) then
			Duel.BreakEffect()
