...
function s.initial_effect(c)
	--Banish up to 5 cards from your RP face-down
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,LOCATION_REST,0,1,nil,tp,POS_FACEDOWN,REASON_EFFECT) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local your_rg=Duel.SelectMatchingCard(tp,Card.IsAbleToRemove,tp,LOCATION_REST,0,1,5,nil,tp,POS_FACEDOWN,REASON_EFFECT)
	...
	if Duel.Remove(your_rg,POS_FACEDOWN,REASON_EFFECT)>0 and Duel.IsExistingMatchingCard(Card.IsFacedown,tp,LOCATION_REMOVED,0,7,nil)
		and Duel.IsExistingMatchingCard(Card.IsAbleToRemove,tp,0,LOCATION_REST,1,nil,1-tp,POS_FACEDOWN,REASON_EFFECT) then
		local oppo_rg=Duel.GetMatchingGroup(Card.IsAbleToRemove,tp,0,LOCATION_REST,nil,1-tp,POS_FACEDOWN,REASON_EFFECT)
		if #oppo_rg>5 then
