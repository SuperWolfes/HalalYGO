...
function s.initial_effect(c)
	...
	c:RegisterEffect(e0)
	--Place 1 monster from the field in the Actional & Trap Zone as a Continuous Actional
...
function s.hspfilter(c)
	return c:IsMonsterCard() and c:IsFaceup() and c:IsAbleToRestAsCost()
end
...
function s.plfilter(c)
	local p=c:GetOwner()
	return c:IsFaceup() and Duel.GetLocationCount(p,LOCATION_SZONE)>0 and not c:IsUnliked() and c:CheckUniqueOnField(p)
end
