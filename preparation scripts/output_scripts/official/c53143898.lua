...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Send 1 face-up "Altergeist" card to the RP and Special Summon 1 "Altergeist" monster from the RP
...
function s.tgfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(SET_ALTERGEIST) and c:IsAbleToRest() and Duel.GetMZoneCount(tp,c)>0
end
