...
function s.cfilter(c)
	return c:IsRace(RACE_TOXIC) and c:GetBaseAttack()>0 and c:IsAbleToRemoveAsCost() and aux.SpElimFilter(c,true)
end
...
function s.filter(c)
	return c:IsFaceup() and c:IsRace(RACE_TOXIC) and c:IsDefenseBelow(0)
end
