...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Send 1 monster your opponent controls to the RP
...
function s.efftg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=Duel.GetMatchingGroupCount(Card.IsRace,tp,LOCATION_REST,0,nil,RACE_TAINTED)
...
function s.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if op==1 then
		--This card gains 800 ATK for each Tainted monster in your RP
		c:UpdateAttack(ct*800,RESETS_STANDARD_DISABLE)
	elseif op==2 then
		--Inflict 500 damage to your opponent for each Tainted monster in your RP
		Duel.Damage(1-tp,ct*500,REASON_EFFECT)
	end
end
function s.tgtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRest,tp,0,LOCATION_MZONE,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOREST,nil,1,1-tp,LOCATION_MZONE)
end
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOREST)
	local g=Duel.SelectMatchingCard(tp,Card.IsAbleToRest,tp,0,LOCATION_MZONE,1,1,nil)
	if #g>0 then
