...
function s.initial_effect(c)
	--Special Summon 1 monster from the opponent's RP to their field
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_SPECIAL_SUMMON+CATEGORY_REMOVE+CATEGORY_TOREST)
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_TOREST,nil,1,1-tp,LOCATION_REMOVED)
end
function s.tgfilter(c)
	return c:IsMonster() and c:IsAbleToRest() and c:IsFaceup()
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and Duel.SpecialSummon(tc,0,tp,1-tp,false,false,POS_FACEUP)>0
		...
		Duel.BreakEffect()
		Duel.SendtoRest(gg,REASON_EFFECT|REASON_RETURN)
	end
end
