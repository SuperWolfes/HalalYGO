...
function s.spcon(e,c)
	...
	local rg1=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_MZONE+LOCATION_REST+LOCATION_HAND,0,c,RACE_MACHINE)
	local rg2=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_MZONE+LOCATION_REST+LOCATION_HAND,0,c,RACE_BEASTWARRIOR)
	local rg=rg1:Clone()
	rg:Merge(rg2)
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,c)
	local c=e:GetHandler()
	local rg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_MZONE+LOCATION_REST+LOCATION_HAND,0,c,RACE_MACHINE)
	rg:Merge(Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_MZONE+LOCATION_REST+LOCATION_HAND,0,c,RACE_BEASTWARRIOR))
