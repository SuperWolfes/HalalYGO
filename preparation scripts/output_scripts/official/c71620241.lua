...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Place 1 "Advanced Crystal Beast" monster face-up in the S/T as Continuous Actional
...
function s.filter(c)
	return c:IsSetCard(SET_ADVANCED_CRYSTAL_BEAST) and not c:IsUnliked() and (not c:IsLocation(LOCATION_REMOVED) or c:IsFaceup())
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.filter),tp,LOCATION_DECK|LOCATION_REST|LOCATION_HAND|LOCATION_REMOVED,0,1,1,nil)
	local tc=g:GetFirst()
	if tc then
