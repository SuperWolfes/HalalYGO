...
function s.initial_effect(c)
	--Send 2 cards to the RP and add 1 "Goodie" Equip card to the hand
	...
	c:RegisterEffect(e1)
	--Equip 1 "Goodie" card to 1 "Goodie" Link Monster
...
function s.thfilter(c)
	return c:IsSetCard(SET_EVIL_EYE) and c:IsEquipActional() and c:IsAbleToHand()
end
function s.tgfilter(c)
	return c:IsSetCard(SET_EVIL_EYE) and c:IsAbleToRestAsCost()
end
...
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local dg=aux.SelectUnselectGroup(g,e,tp,2,2,s.rescon,1,tp,HINTMSG_TOREST)
	Duel.SendtoRest(dg,REASON_COST)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	--Lose 500 LP each time you activate a non-"Goodie" card or effect
	local e1=Effect.CreateEffect(c)
...
function s.eqfilter(c,tp)
	return c:IsSetCard(SET_EVIL_EYE) and c:IsEquipActional() and c:CheckUniqueOnField(tp)
...
function s.eqptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingMatchingCard(s.eqfilter,tp,LOCATION_REST,0,1,nil,tp) end
...
function s.eqpop(e,tp,eg,ep,ev,re,r,rp)
	...
	local ec=Duel.SelectMatchingCard(tp,s.eqfilter,tp,LOCATION_REST,0,1,1,nil,tp):GetFirst()
