...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Special Summon 1 face-up "Dracoslayer" Pendulum Monster from your Extra Deck
...
function s.spfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xc7) and c:IsType(TYPE_PENDULUM) and Duel.GetLocationCountFromEx(tp,tp,nil,c)>0
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and Duel.SpecialSummon(tc,0,tp,tp,false,false,POS_FACEUP)>0 then
		...
		e1:SetValue(aux.cannotmatfilter(SUMMON_TYPE_XYZ))
