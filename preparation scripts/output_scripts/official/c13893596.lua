--究極封印神エクゾディオス
--Exdudius the Ultimate Unliked Watcher
local s,id=GetID()
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Send 1 monster from your hand or Deck to the RP
	...
	c:RegisterEffect(e3)
	--Gains 1000 ATK for each Normal Monster in your RP
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_SINGLE)
	e4:SetProperty(EFFECT_FLAG_SINGLE_RANGE)
	e4:SetCode(EFFECT_UPDATE_ATTACK)
	e4:SetRange(LOCATION_MZONE)
	e4:SetValue(function(e,c) return Duel.GetMatchingGroupCount(Card.IsType,c:GetControler(),LOCATION_REST,0,nil,TYPE_NORMAL)*1000 end)
	c:RegisterEffect(e4)
...
function s.winfilter(c,rc)
	return c:IsRelateToCard(rc) and c:IsSetCard(SET_UNLIKED_ONE) and c:IsMonster()
end
function s.tgop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOREST)
	local tc=Duel.SelectMatchingCard(tp,aux.AND(Card.IsMonster,Card.IsAbleToRest),tp,LOCATION_HAND|LOCATION_DECK,0,1,1,nil):GetFirst()
	if tc and Duel.SendtoRest(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_REST) and c:IsRelateToEffect(e) then
		...
		local g=Duel.GetMatchingGroup(s.winfilter,tp,LOCATION_REST,0,nil,c)
