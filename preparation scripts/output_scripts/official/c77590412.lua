...
function s.initial_effect(c)
	-- Send 1 EARTH Warrior from your Deck to the RP
	...
	e4:SetCategory(CATEGORY_TOREST+CATEGORY_SPECIAL_SUMMON)
...
function s.tgfilter(c)
	return c:IsAttribute(ATTRIBUTE_EARTH) and c:IsRace(RACE_WARRIOR) and c:IsAbleToRest()
end
...
function s.atktg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	if chkc then return chkc:IsLocation(LOCATION_REST) and chkc:IsControler(tp) and s.atkfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetMZoneCount(tp,c)>0
		and Duel.IsExistingTarget(s.atkfilter,tp,LOCATION_REST,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tc=Duel.SelectTarget(tp,s.atkfilter,tp,LOCATION_REST,0,1,1,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_TOREST,c,1,tp,0)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,tc,1,tp,0)
end
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) or Duel.SendtoRest(c,REASON_EFFECT)==0 or not c:IsLocation(LOCATION_REST) then return end
