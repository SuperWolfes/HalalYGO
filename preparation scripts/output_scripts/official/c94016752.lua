...
function s.initial_effect(c)
	--Make your opponent send a monster of the declared Type and Attribute they control to the RP
...
function s.tgfilter(c,race,attr)
	return c:IsFaceup() and c:IsRace(race) and c:IsAttribute(attr) and c:IsAbleToRest()
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(sc,REASON_RULE,PLAYER_NONE,1-tp)>0 then
