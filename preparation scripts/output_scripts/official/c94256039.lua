...
function s.initial_effect(c)
	...
	e5:SetCode(EVENT_ADD_COUNTER+COUNTER_ACTIONAL)
...
function s.acop(e,tp,eg,ep,ev,re,r,rp)
	...
	if re:IsHasType(EFFECT_TYPE_ACTIVATE) and re:IsActiveType(TYPE_ACTIONAL) and c:GetFlagEffect(1)>0 then
		c:AddCounter(COUNTER_ACTIONAL,1)
		if c:GetCounter(COUNTER_ACTIONAL)==4 then
...
function s.descon(e,tp,eg,ep,ev,re,r,rp)
	return re:GetHandler()~=e:GetHandler() and e:GetHandler():GetCounter(COUNTER_ACTIONAL)>=4
end
