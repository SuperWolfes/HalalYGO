...
function s.initial_effect(c)
	-- Banish 1 Field Actional
	local e1=Effect.CreateEffect(c)
	e1:SetDescription(aux.Stringid(id,0))
	e1:SetCategory(CATEGORY_REMOVE+CATEGORY_LEAVE_REST)
...
function s.rmfilter(c)
	return c:IsFaceup() and c:IsAbleToRemove() and not c:IsUnliked()
end
function s.rmtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_LEAVE_REST,nil,1,1-g:GetFirst():GetControler(),LOCATION_REST)
end
...
function s.rmop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tc:IsRelateToEffect(e)
		...
		or tc:IsUnliked() then return end
	Duel.BreakEffect()
	if not s.placefield(tc,tp,1-old_fp) then return end
	local g=Duel.GetMatchingGroup(s.plfilter,tc:GetControler(),LOCATION_REST,0,nil,old_fp,tc:GetOriginalCode())
