...
function s.initial_effect(c)
	...
	c:RegisterEffect(e3)
	--Cards in the RP cannot be banished
	...
	c:RegisterEffect(e9)
	--Negate on resolution if an effect would move a card in the RP (other than itself)
...
function s.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not res and s.discheck(ev,CATEGORY_LEAVE_REST,re,im0,im1,targets) then res=true end
