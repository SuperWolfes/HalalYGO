...
function s.initial_effect(c)
	--Place this card and 2 cards from your opponent's RP on their field
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(nil,tp,0,LOCATION_REST,2,nil) and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>2 end
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_REST,nil,2,0,0)
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(nil,tp,0,LOCATION_REST,nil)
	...
	if #g>1 then
		...
		if rsel:IsContains(c) then
			...
			sg:RemoveCard(rsel+c)
            		Duel.SendtoRest(rsel+c,REASON_EFFECT)
