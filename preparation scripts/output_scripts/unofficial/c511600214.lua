...
function s.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:GetFlagEffect(id)>0 then
		tc:SetFlagEffectLabel(id,tc:GetFlagEffectLabel(id)+Duel.GetBattleDamanced(ep))
	else
		tc:RegisterFlagEffect(id,RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_END,0,1,Duel.GetBattleDamanced(ep))
	end
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if ec and ec:IsFaceup() then
		...
		e3:SetValue(aux.ChangeBattleDamanced(0,DOUBLE_DAMAGE))
