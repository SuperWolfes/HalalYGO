--オッドアイズ・ランサー・ドラゴン (Anime)
--Odd-Eyes Lancer Dragon (Anime)
local s,id=GetID()
...
function s.dam2op(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetBattleDamanced(tp)==0 then return end
	Duel.ChangeBattleDamanced(tp,Duel.GetBattleDamanced(tp)/2)
	local e1=Effect.CreateEffect(e:GetHandler())
...
function s.dmgop(e,tp,eg,ep,ev,re,r,rp)
	if ep==tp then
		Duel.Damanced(1-tp,ev,REASON_EFFECT)
	end
end
