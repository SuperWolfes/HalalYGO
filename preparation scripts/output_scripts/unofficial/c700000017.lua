...
function s.pacon(e,tp,eg,ep,ev,re,r,rp)
	...
	return Duel.GetTurnPlayer()~=tp and ph>=0x08 and ph<=0x20 
		and (Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamancedCalculated()) 
end
function s.pafil(c)
...
function s.macon(e,tp,eg,ep,ev,re,r,rp)
	local ph=Duel.GetCurrentPhase()
	return ph>=0x08 and ph<=0x20 and (Duel.GetCurrentPhase()~=PHASE_DAMAGE or not Duel.IsDamancedCalculated()) 
		and s[0]>0
end
function s.mafil(c)
