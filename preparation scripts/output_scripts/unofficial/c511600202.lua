...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Destroy(Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_MZONE,0,nil),REASON_EFFECT)>0
		and Duel.Damanced(tp,atk,REASON_EFFECT,true) and Duel.Damanced(1-tp,atk,REASON_EFFECT,true) then
		Duel.RDComplete()
		...
		if #dg>0 then
			...
			if #og>0 then
				Duel.Damanced(1-tp,og:GetFirst():GetPreviousAttackOnField()/2,REASON_EFFECT)
			end
		end
	end
