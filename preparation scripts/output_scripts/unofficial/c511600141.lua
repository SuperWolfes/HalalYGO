...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if ac and (Duel.GetCurrentPhase()<PHASE_DAMAGE_CAL or not Duel.IsDamancedCalculated()) then
...
function s.bttg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ac:IsFaceup() and (ac:IsAttackPos() or ac:IsHasEffect(EFFECT_DEFENSE_ATTACK))
		...
		and not ac:IsHasEffect(EFFECT_UNLIKED) and not ac:IsHasEffect(EFFECT_CANNOT_ATTACK)
...
function s.btop(e,tp,eg,ep,ev,re,r,rp)
	Duel.CalculateDamanced(e:GetLabelObject():GetFirst(),e:GetLabelObject():GetNext())
	e:Reset()
end
