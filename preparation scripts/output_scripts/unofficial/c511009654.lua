...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return not Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) 
			and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0x56f,TYPES_TOKEN,0,0,1,RACE_TOXIC,ATTRIBUTE_DARK)
	end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_GUARDIAN) or Duel.GetLocationCount(tp,LOCATION_MZONE)<=2 
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0x56f,TYPES_TOKEN,0,0,1,RACE_TOXIC,ATTRIBUTE_DARK) then return end
	for i=1,3 do
