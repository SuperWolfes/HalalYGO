...
function s.con1(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetBattleDamanced(tp)>0
end
function s.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:GetAttack()>=Duel.GetBattleDamanced(tp) and not c:IsRelateToBattle() end
end
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SelectEffectYesNo(tp,c) then
		...
		c:SetFlagEffectLabel(id,c:GetFlagEffectLabel(id)+Duel.GetBattleDamanced(tp))
