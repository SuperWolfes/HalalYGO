...
function s.initial_effect(c)
	--Activate by sending 1 "Endless Emptiness" from your field to the RP
	...
	c:RegisterEffect(e2)
	--"Timewatcher" monsters you control cannot activate their effects in the Standby Phase
	...
	c:RegisterEffect(e3)
	--Allows you to control more than 1 "Timewatcher" monster
	local e4=Effect.CreateEffect(c)
	e4:SetType(EFFECT_TYPE_FIELD)
	e4:SetRange(LOCATION_SZONE)
	e4:SetCode(513000047)
	e4:SetTargetRange(LOCATION_MZONE,0)
	e4:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,SET_TIMEWATCHER))
	c:RegisterEffect(e4)
	--Special Summon "Sephylon, the Ultimate Timewatcher" from your hand, Deck or RP
	...
	c:RegisterEffect(e5)
	--"Timewatcher" monster Summon register
...
function s.aclimit(e,re,tp)
	...
	return re:GetActivateLocation()==LOCATION_MZONE and rc:IsSetCard(SET_TIMEWATCHER) and not rc:IsImmuneToEffect(e)
end
function s.cfilter(c,tp)
	return c:IsSetCard(SET_TIMEWATCHER) and c:IsFaceup() and c:IsSummonPlayer(tp)
end
...
function s.sephop(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.sephspfilter),tp,LOCATION_HDG,0,1,1,nil,e,tp)
	if #g>0 then
