...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) and tc and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		...
		if Duel.CheckEvent(EVENT_ATTACK_ANNOUNCE) and tp~=Duel.GetTurnPlayer() then
			if Duel.GetAttackTarget()==tc and Duel.IsExistingTarget(Card.IsMonster,tp,LOCATION_REST,0,1,nil) then
				local atc=Duel.GetMatchingGroup(Card.IsMonster,tp,LOCATION_REST,0,nil)
...
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(Card.IsMonster,tp,LOCATION_REST,0,1,nil) end
	local tc=Duel.GetMatchingGroup(Card.IsMonster,tp,LOCATION_REST,0,nil)
