...
function s.cfilter(c)
	return c:IsRace(RACE_PYRO) and c:IsAbleToRestAsCost()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(1-tp,LOCATION_MZONE,tp)>1 
		and Duel.IsExistingTarget(s.filter,tp,0,LOCATION_REST,2,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.filter,tp,0,LOCATION_REST,2,2,nil,e,tp)
