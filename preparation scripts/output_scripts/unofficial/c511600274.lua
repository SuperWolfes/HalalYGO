...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_HAND+LOCATION_REST,0,1,nil,e,tp,zones) end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.RestValleyFilter(s.spfilter),tp,LOCATION_HAND+LOCATION_REST,0,1,math.min(ft,2),nil,e,tp,zones)
	if #g>0 then
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.Destroy(g,REASON_EFFECT)>0 then
		...
		Duel.Damanced(1-tp,dam,REASON_EFFECT)
	end
end
