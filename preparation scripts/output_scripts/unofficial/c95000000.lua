	...
	Duel.Win=(function()
		...
		return function(tp,r)
			...
			if tp==0 and boss1 then
				Duel.Damanced(1,startlps,REASON_RULE)
				return
			elseif tp==1 and boss0 then
				Duel.Damanced(0,startlps,REASON_RULE)
				return
			elseif tp==PLAYER_NONE or tp==PLAYER_ALL then
				if boss0 and boss1 then
					Duel.Damanced(0,startlps,REASON_RULE,true)
					Duel.Damanced(1,startlps,REASON_RULE,true)
					Duel.RDComplete()
				elseif boss0 then
					Duel.Damanced(0,startlps,REASON_RULE)
					f(1,r)
...
function s.arrange(_,tp)
	for i=3,0,-1 do
		Duel.GetMatchingGroup(function(c) return c.mark==i end,tp,LOCATION_DECK,0,nil):ForEach(Duel.MoveToDeckTop)
	end
end
function s.stop(e)
	...
	if Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_REMOVED,0,1,nil,id) then
		...
		startlps=Duel.GetLP(tp)
		Duel.SetLP(tp,startlps*Duel.GetPlayersCount(1-tp))
		...
		s.arrange(nil,tp)
		if hct>0 then Duel.Draw(tp,hct,REASON_EFFECT) end
	end
end
