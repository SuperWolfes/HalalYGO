...
function s.refcon(e,tp,eg,ep,ev,re,r,rp)
	...
	return bc and bc:IsLevelBelow(4) and bc:IsControler(1-tp) and Duel.GetBattleDamanced(tp)>0
end
function s.refop(e,tp,eg,ep,ev,re,r,rp)
	Duel.ChangeBattleDamanced(1-tp,Duel.GetBattleDamanced(1-tp)+Duel.GetBattleDamanced(tp),false)
	Duel.ChangeBattleDamanced(tp,0)
end
function s.ndcon(e,tp,eg,ep,ev,re,r,rp)
	local ec=e:GetHandler():GetEquipTarget()
	local bc=ec:GetBattleTarget()
	if not bc or Duel.GetBattleDamanced(ec:GetControler())<=0 or not bc then return false end
