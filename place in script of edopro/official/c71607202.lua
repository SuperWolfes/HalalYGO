...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	--Special Summon 1 Tainted monster from GY
...
function s.cfilter(c,e)
	return c:IsRace(RACE_TAINTED) and c:IsReleasableByEffect(e)
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	--Cannot Special Summon for the rest of the turn, except Tainted monsters
	...
	if Duel.DiscardHand(tp,nil,1,1,REASON_EFFECT+REASON_DISCARD)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		local tc=Duel.GetFirstTarget()
		if tc:IsRelateToEffect(e) and tc:IsRace(RACE_TAINTED) then
