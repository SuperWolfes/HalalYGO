...
function s.costfilter(c,tp)
	...
	and not c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsSetCard(0xdc) and c:IsMonster() and c:IsAbleToRestAsCost() and Duel.GetMZoneCount(tp,c)>0
end
...
function s.tgfilter(c)
	return c:IsMonster() and c:IsSetCard(0xdc) and c:IsAbleToRest()
end
...
function s.thtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter2,tp,LOCATION_DECK+LOCATION_REST,0,1,nil) end
...
function s.tgop2(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.thfilter2),tp,LOCATION_DECK+LOCATION_REST,0,1,1,nil)
	if #g>0 then
