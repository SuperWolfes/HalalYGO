...
function s.exfilter(c,e,tp,ft)
	return c:IsType(TYPE_SYNCHRO) and c.material 
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REST+LOCATION_DECK,0,1,nil,e,tp,c,ft)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(s.spfilter),tp,LOCATION_REST+LOCATION_DECK,0,1,1,nil,e,tp,cg:GetFirst(),ft):GetFirst()
	if tc then
