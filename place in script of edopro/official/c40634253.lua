...
function s.damcon1(e,tp,eg,ep,ev,re,r,rp)
	...
	return not tc:IsPreviousControler(tp) and tc:IsLocation(LOCATION_REST)
...
function s.damfilter1(c,tp)
	return c:IsReason(REASON_EFFECT) and c:IsReason(REASON_DESTROY) and c:IsLocation(LOCATION_REST) and not c:IsPreviousControler(tp)
end
...
function s.damfilter2(c,tp)
	return c:IsReason(REASON_BATTLE+REASON_EFFECT) and c:IsReason(REASON_DESTROY) and c:IsLocation(LOCATION_REST) and not c:IsPreviousControler(tp)
end
