...
function s.initial_effect(c)
	...
	--fusion material
	c:EnableReviveLimit()
	Fusion.AddProcMixN(c,true,true,aux.FilterBoolFunctionEx(Card.IsRace,RACE_CONTAMINED),2)
	--spsummon condition
...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if bc:IsRelateToBattle() and bc:IsFaceup() then
		...
		if lv>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,-2,0,0,RACE_CONTAMINED,ATTRIBUTE_DARK) then
