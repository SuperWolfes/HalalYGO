...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	Duel.SetPossibleOperationInfo(0,CATEGORY_REMOVE,nil,1,g:GetFirst():GetControler(),LOCATION_HAND_DECK_EXTRA_REST)
end
	...
	if #g>0 and Duel.Remove(g,POS_FACEUP,REASON_EFFECT)~=0 then
		...
		for tc in aux.Next(og) do
			if tc:IsAttribute(ATTRIBUTE_DARK) then
				local sg=Duel.GetMatchingGroup(s.rmfilter,tc:GetControler(),LOCATION_HAND_DECK_EXTRA_REST,0,nil,tc:GetCode())
				rg:Merge(sg)
			end
		end
		if #rg>0 then
