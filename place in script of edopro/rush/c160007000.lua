...
function s.cfilter(c,e,tp)
	return c:IsFaceup() and c:IsCode(CARD_UMI) and c:IsAbleToRestAsCost() and
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SendtoRest(g,REASON_COST) then
		local g=Duel.GetMatchingGroup(aux.FilterMaximumSideFunctionEx(aux.TRUE),tp,LOCATION_ONFIELD,LOCATION_ONFIELD,e:GetHandler())
		g=g:AddMaximumCheck()
