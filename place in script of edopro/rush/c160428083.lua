--ビックリ・ゾンビクトリー
--Surprising Contaminated Victory
--scripted by Naim
local s,id=GetID()
...
function s.filter(c)
	return c:IsRace(RACE_CONTAMINED) and c:IsType(TYPE_NORMAL)
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and Duel.ChangePosition(g,POS_FACEUP_DEFENSE,POS_FACEDOWN_DEFENSE,POS_FACEUP_ATTACK,POS_FACEUP_ATTACK)>0 then
		local atkg=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsRace,RACE_CONTAMINED),tp,LOCATION_MZONE,0,nil)
