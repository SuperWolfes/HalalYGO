...
function s.thfilter(c,tp)
	return c:IsType(TYPE_NORMAL) and c:IsRace(RACE_MENTAL) and c:IsAbleToHand()
	and Duel.IsExistingMatchingCard(s.tdfilter,tp,LOCATION_REST,0,2,c,tp)
end
...
function s.rescon(sg,e,tp,mg)
	return Duel.IsExistingMatchingCard(s.thfilter2,tp,LOCATION_REST,0,1,sg,tp)
end