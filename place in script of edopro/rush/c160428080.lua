...
function s.posfilter(c)
	return c:IsRace(RACE_CONTAMINED) and c:IsAttackPos() and c:IsType(TYPE_NORMAL) and c:IsCanChangePosition() and c:IsFaceup()
end
...
function s.defposfilter(c)
	return c:IsRace(RACE_CONTAMINED) and c:IsPosition(POS_FACEUP_DEFENSE)
end
