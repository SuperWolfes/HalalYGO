...
function s.cfilter(c,tp)
	return c:IsAbleToDeck() and Duel.IsExistingMatchingCard(s.cfilter2,tp,LOCATION_REST,0,2,c,c:GetRace())
end
...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(s.filter,1,nil,e,tp) and Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REST,0,1,nil,tp)
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return 
		 Duel.IsExistingMatchingCard(s.cfilter,tp,LOCATION_REST,0,1,nil,tp)
	end
