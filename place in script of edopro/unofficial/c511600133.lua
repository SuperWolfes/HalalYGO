--機械天使の絶対儀式 (Anime)
--Machine Angel Absolute Locked (Anime)
...
function s.mfilterf(c,tp,mg,rc)
	if c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:GetSequence()<5 then
		...
		return mg:CheckWithSumEqual(Card.GetLockedLevel,rc:GetLevel(),0,99,rc)
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg1=Duel.GetLockedMaterial(tp)
		local mg2=Duel.GetMatchingGroup(s.mfilter,tp,LOCATION_REST,0,nil)
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc then
		local mg=mg1:Filter(Card.IsCanBeLockedMaterial,tc,tc)
		mg:Merge(mg2)
		local mat=nil
		if ft>0 then
			...
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
			local mat2=mg:SelectWithSumEqual(tp,Card.GetLockedLevel,tc:GetLevel(),0,99,tc)
			mat:Merge(mat2)
		end
		tc:SetMaterial(mat)
		local mat2=mat:Filter(Card.IsLocation,nil,LOCATION_REST):Filter(Card.IsRace,nil,RACE_WARRIOR+RACE_WANDERER)
		mat:Sub(mat2)
