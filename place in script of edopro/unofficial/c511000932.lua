...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		local sg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_REST,0,nil,e,tp,tc:GetLevel())
		if tc:IsLocation(LOCATION_REST) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
			and #sg>0 then
			Duel.BreakEffect()
